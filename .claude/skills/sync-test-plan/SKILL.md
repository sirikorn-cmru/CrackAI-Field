...
และ **ระบุตัวเลือกที่แนะนำ** เป็นตัวเลือกแรก ต่อท้าย label ด้วย "(แนะนำ)" พร้อมเหตุผล

## ขอบเขตงาน (scope) — ระบุเจาะจงได้

ค่าเริ่มต้นคือครอบคลุมทุกรหัสใน `backlog.md` และทั้ง 3 เอกสาร แต่ผู้ใช้ระบุเจาะจงได้หลายระดับ:
**ทั้งระบบ** · **ตามบทบาทผู้ใช้** · **ตาม journey** · **ตามฟีเจอร์** · **ตามรหัส FR/NFR** ·
**ตามระดับความสำคัญ** (เช่น เฉพาะ "สูง") · **ตามเอกสาร** (เช่น ขอเฉพาะ `test-plan.md`)

- ถ้าผู้ใช้ระบุ scope มาชัดเจนแล้ว → ส่งต่อไปใน prompt ของ subagent ตรงๆ **ห้ามถามซ้ำ**
- ถ้าไม่ได้ระบุ → ใช้ค่าเริ่มต้น (ครบทุกอย่าง) ไม่ต้องถาม เพราะเป็นค่าที่ปลอดภัยที่สุด
- ถ้าระบุมาแบบกำกวมว่าหมายถึงระดับใด → ถามก่อน โดยสร้างตัวเลือกจากเนื้อหาจริงใน
  `feature-list.md`/`backlog.md` ไม่ใช่ตัวเลือกลอยๆ
- **เมื่อ scope ถูกจำกัด ต้องรายงานให้ผู้ใช้ทราบเสมอว่าอะไรยังไม่ถูกครอบคลุมในรอบนี้**
...
6. **ตรวจสอบผลลัพธ์ก่อนรายงาน** — นอกจากการสุ่มตรวจปกติ **ต้องตรวจ 3 จุดนี้ทุกครั้ง**:
   - `test-plan.md` **มีหัวข้อ Risk Management จริง** และความเสี่ยงที่เขียนไว้ derive จาก NFR
     ของโปรเจกต์นี้จริง ไม่ใช่รายการความเสี่ยงทั่วไปที่ใช้กับโปรเจกต์ไหนก็ได้
   - ทุกไฟล์ใน `test-cases/` **มีคอลัมน์ Test Data ครบทุกแถว ไม่มีช่องว่าง**
     (ถ้าไม่ต้องใช้ข้อมูลต้องเขียนว่า "ไม่มี")
   - ทุก test case มี **reference กลับไปยัง AC และรหัส FR/NFR** ที่มีอยู่จริง
   รายละเอียดการตรวจอื่น: หลัง subagent รายงานว่าแก้ไขอะไรไปบ้าง (รอบสุดท้าย) ให้สุ่ม

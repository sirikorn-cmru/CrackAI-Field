...
โปรเจกต์นี้**เริ่มมีซอร์สโค้ดแล้วตั้งแต่ 2026-09-03** (Phase 1) หลังจากเป็นพื้นที่ requirements/design ล้วนมาก่อนหน้านั้น (**โดเมนของระบบดูจากเอกสารใน `docs/01-requirements/01-spec/` เท่านั้น ห้ามสมมติ** — ดูหัวข้อถัดไป) ความคืบหน้าของแต่ละขั้นตอนไม่เท่ากัน — บางไฟล์มีเนื้อหาแล้ว บางไฟล์/โฟลเดอร์ยังว่างรอเนื้อหาอยู่ ให้ตรวจสถานะจริงของแต่ละไฟล์ก่อนอ้างอิงหรือแก้ไข อย่าเชื่อคำอธิบายสถานะที่เขียนไว้ในเอกสารฉบับเก่า

### คำสั่งที่ใช้จริง (รันจาก root)

| คำสั่ง | ทำอะไร |
|---|---|
| `npm install` | ติดตั้ง dependency และเชื่อม workspace |
| `npm run build` | build `@crackai/shared` (workspace เดียวที่ต้อง emit) |
| `npm run typecheck` | build shared แล้วตรวจชนิดข้อมูลทุก workspace |
| `npm test` | build shared แล้วรันเทสต์ทุก workspace |

ยังไม่มี linter และยังไม่มี CI — เมื่อเพิ่มแล้วให้กลับมาอัปเดตตารางนี้

### โครงสร้างซอร์สโค้ด

```
packages/shared/     ชนิดข้อมูลร่วม (บทบาท/สถานะ/entity) คัดลอกค่าตรงตัวจาก db-spec
apps/server/
  src/domain/        ตรรกะธุรกิจล้วน ไม่ผูก HTTP/ฐานข้อมูล — 1 service ต่อกลุ่ม operation ใน api-spec
  src/infra/         การใช้งานจริงของ port (crypto, ที่เก็บข้อมูล)
  db/migrations/     schema จริงบน MySQL/MariaDB
  test/              เทสต์ที่อ้างอิงรหัส T-x-xx และ FR/NFR กลับไปยังเอกสาร
```

**ชั้น domain ไม่รู้จัก HTTP โดยเจตนา** เพราะ `technology-stack.md` ตัดสิน runtime (Node.js/TypeScript) แล้วแต่**ยังไม่ได้ตัดสิน protocol/web framework** — `api-spec.md` จึงเขียนเป็น operation contract ไม่ใช่ endpoint ชั้นขนส่งจะถูกเพิ่มเมื่อมีการตัดสินใจ

**ค่า enum ภาษาไทยในโค้ดต้องตรงกับ `db-spec.md` ทุกตัวอักษร** ห้ามแปลเป็นภาษาอังกฤษ เพราะเอกสารทุกชั้นและ test case อ้างค่าเหล่านี้ ถ้าจำเป็นต้องเปลี่ยน ให้แก้เอกสารต้นทางก่อนเสมอ
...
**ข้อจำกัดที่ต้องรู้: subagent ในโปรเจกต์นี้ถามผู้ใช้ไม่ได้จริง** แม้จะมี `AskUserQuestion` อยู่ในรายการเครื่องมือก็ตาม ผลที่ตามมามีสองข้อ: (1) ทุก agent ที่อาจต้องถาม ต้องมี**ทางถอย** — ให้รายงานทางเลือกทั้งหมดกลับไปให้ skill ที่เรียก โดยยังไม่เขียนส่วนที่ต้องตัดสินใจ แล้ว skill เป็นคนถามผู้ใช้แทน **ห้ามเดาเอง** (2) **skill ที่เป็น orchestrator ต้องอยู่ในเทรดหลักเสมอ ห้ามยกไปเป็น subagent** เพราะจะทำให้ทั้งสายงานถามผู้ใช้ไม่ได้เลย

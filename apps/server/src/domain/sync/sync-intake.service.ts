...
export class SyncIntakeService {
  ...
  /**
   * @param mergeStrategy **ไม่มีค่าเริ่มต้นโดยเจตนา** แม้กลยุทธ์จะถูกตัดสินแล้วเมื่อ 2026-09-04
   *   (`manualOnlyMergeStrategy`) — การบังคับให้จุดประกอบระบบเลือกอย่างชัดแจ้งทำให้การเปลี่ยน
   *   กลยุทธ์ในอนาคตเป็นการแก้ที่เดียวและมองเห็นได้ ไม่ใช่ค่าเริ่มต้นที่ไหลตามไปโดยไม่มีใครรู้
   */
  constructor(
 ...
 *
 * **ขอบเขตที่ต้องรู้ (NFR-03 รองรับบางส่วนโดยเจตนา)**: การเทียบอยู่ที่ระดับ aggregate เท่านั้น
 * ไม่เปรียบเทียบค่าระดับฟิลด์ สองอุปกรณ์ที่แก้คนละฟิลด์ของอาคารเดียวกันจึงถูกนับเป็นความขัดแย้ง
 * และส่งให้มนุษย์ ทั้งที่ในทางทฤษฎี merge ได้ — ดูเหตุผลและเงื่อนไขที่จะกลับมาทำที่
 * `manualOnlyMergeStrategy` ใน merge-strategy.ts
 *

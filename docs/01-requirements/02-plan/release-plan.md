...
อัปเดต 2026-09-24: `nfr-reviewer` ตรวจรอบที่ 11 แล้วลดสถานะ **NFR-03 จาก "รองรับแล้ว" เป็น "รองรับบางส่วน" ใน Phase 1** เพราะกลยุทธ์ที่ผู้ใช้ตัดสินใจคือส่งทุกกรณีที่ตรวจพบการชนกันของเวอร์ชันให้มนุษย์ตัดสินใจเสมอ (ตรวจจับที่ระดับ aggregate เท่านั้น) ยังไม่มี merge อัตโนมัติระดับฟิลด์เมื่อสองอุปกรณ์แก้คนละฟิลด์ที่ไม่ขัดแย้งกันจริงตามที่ NFR-03 ตั้งเป้าไว้เต็มรูปแบบ — ไม่มีรหัสถูกย้าย phase หรือเพิ่ม/ลด task ใดๆ จากอัปเดตนี้ ดูรายละเอียดที่หมายเหตุท้าย Exit criteria ของ Phase 1 ด้านล่าง และ [[phase-1-foundation-auth-offline-tasks]] แถว T-1-08
...
- เมื่อผู้ใช้กรอก PIN ผิดติดต่อกันครบ 5 ครั้ง ระบบล้าง credential/PIN/เซสชันที่แคชไว้บนอุปกรณ์ทันทีและบังคับให้เข้าสู่ระบบใหม่ด้วยรหัสผ่านเมื่อมีสัญญาณ โดยข้อมูลแบบสำรวจและคิวรอซิงค์ที่บันทึกไว้บนอุปกรณ์เดียวกันต้องไม่ถูกลบหรือกระทบจากการล้างนี้โดยเด็ดขาด (ดูรายละเอียดที่ [[phase-1-foundation-auth-offline-tasks]] แถว T-1-12)
...
- ระบบตรวจจับความขัดแย้งของข้อมูลที่แก้ไขจากหลายอุปกรณ์ที่ระดับ aggregate (เทียบ `data_version`/`last_modified_at`/`last_modified_by_device_id`) และตั้งสถานะรอแก้ไขด้วยมือทุกครั้งที่ตรวจพบการชนกันของเวอร์ชัน — **รองรับ NFR-03 บางส่วนใน Phase 1 นี้ (ดูหมายเหตุด้านล่าง)**
...
**หมายเหตุ NFR-03 — รองรับบางส่วนใน Phase 1 (อัปเดต 2026-09-24 จาก `nfr-reviewer` รอบที่ 11):** ถ้อยคำเต็มของ NFR-03 ต้องการทั้งการตรวจจับความขัดแย้ง **และ** การรวม (merge) ข้อมูลที่แก้ไขจากหลายอุปกรณ์เข้าด้วยกันโดยอัตโนมัติเมื่อไม่ขัดแย้งกันจริง (เช่น สองอุปกรณ์แก้คนละฟิลด์ของระเบียนเดียวกัน) Phase 1 นี้ส่งมอบเฉพาะการตรวจจับที่ระดับ aggregate และส่งทุกกรณีที่พบให้มนุษย์ตัดสินใจเสมอ ตามกลยุทธ์ที่ผู้ใช้ตัดสินใจไว้เมื่อ 2026-09-04 ส่วน merge อัตโนมัติระดับฟิลด์ยังไม่ถูกส่งมอบโดยการตัดสินใจ ไม่ใช่การมองข้าม (ดู [[phase-1-foundation-auth-offline-tasks]] แถว T-1-08) เงื่อนไขที่จะกลับมาทำส่วนที่เหลือ: (1) มีข้อมูลจริงว่าคิวรอแก้ไขด้วยมือยาวเกินกว่าหัวหน้าผู้สำรวจ/ผู้ดูแลระบบจะรับไหวในทางปฏิบัติ หรือ (2) พบว่าการแก้ไขคนละฟิลด์ของระเบียนเดียวกันพร้อมกันจากหลายอุปกรณ์เกิดขึ้นบ่อยจนกระทบการทำงานจริง

...
> **หมายเหตุสำคัญ (ปรับปรุง 2026-09-03, เพิ่มเติม 2026-09-04)**: [[technology-stack]] มีเนื้อหาแล้ว — ตัดสินใจไปแล้ว 6 ชั้น: Field Client Framework = bare React Native, Local Persistent Store = SQLite ผ่าน `op-sqlite`, Server-side Runtime = Node.js/TypeScript, Primary Data Store = MySQL/MariaDB, Media/Object Storage = Local filesystem บนเซิร์ฟเวอร์, **Protocol/Web Framework = REST over HTTP + Fastify (ปิดเพิ่มเมื่อ 2026-09-04)** (ดู [[technology-stack#3. การตัดสินใจรายชั้น|technology-stack หัวข้อ 3]]) **ประเด็น protocol/web framework จึงปิดแล้ว** (ย่อหน้านี้เคยระบุไว้ก่อนหน้านี้ว่าผู้ใช้ยังไม่เลือก REST/GraphQL/gRPC ใดๆ — ตอนนี้ปิดแล้วว่าเป็น REST over HTTP + Fastify) **อย่างไรก็ตาม เอกสารนี้ยังคงจงใจไม่เพิ่ม HTTP method/path ระบุต่อ operation ในรอบนี้** เพราะเป็นงานที่กระทบทั้ง ~40 operation ในเอกสารนี้และ `detailed-design/` 17 ไฟล์พร้อมกัน จึงแยกเป็นรอบทำงานของตัวเองในอนาคตเพื่อไม่ให้ปนกับการปิดตัวเลข/กลยุทธ์อื่นของรอบนี้ (ดูหัวข้อ 16) แต่ละ operation จึงยังคงอธิบายด้วย: ชื่อ operation, ผู้เรียกได้ (บทบาท), input, output, กฎทางธุรกิจ/validation, กรณี error หลัก และรหัส FR/NFR ที่รองรับ เหมือนเดิม
...
- **กฎทางธุรกิจ**: ต้องรองรับไฟล์ภาพขนาดใหญ่แบบทำต่อได้เมื่อสัญญาณขาดหาย (NFR-02) และรองรับหลายอุปกรณ์/หลายทีมซิงค์พร้อมกันจำนวนมาก (NFR-07) โดยไม่บล็อกการทำงานอื่นของ Field Client — คำขอซิงค์เข้าคิวผ่าน [[architecture#2. Logical Component|คิวรับคำขอซิงค์ฝั่งเซิร์ฟเวอร์ (Server-side Sync Intake Queue)]] ก่อนประมวลผลเพื่อจัดลำดับ/จำกัดอัตราไม่ให้ระบบล่มเมื่อหลายทีมซิงค์พร้อมกันหลังเกิดเหตุ — **กลไกจำกัดอัตราปิดแล้วเมื่อ 2026-09-04**: จำกัดโควตาแบบ token bucket ต่อ `device_id` ร่วมกับจำกัดจำนวนตัวประมวลผล (worker) ที่ทำงานพร้อมกันฝั่งเซิร์ฟเวอร์ — เมื่ออุปกรณ์ใดส่งคำขอเกินโควตา เซิร์ฟเวอร์ต้องปฏิเสธคำขอนั้นชั่วคราวพร้อมแจ้งสถานะ "ขอมากเกินไป" และระยะเวลาที่แนะนำให้ลองใหม่ (ดู [[architecture#6.12 กลไกจัดคิว/จำกัดอัตราคำขอซิงค์ต่ออุปกรณ์ (NFR-07)|architecture §6.12]] — **ตัวเลขอัตราจริงยังไม่ปิด ต้องจูนจาก load test — ห้ามเดาตัวเลข** ดูหัวข้อ 16); ใช้ `client_generated_id` เป็น idempotency key กันบันทึกซ้ำสำหรับทุก entity ที่ระบุไว้ใน Input — **แต่การตรวจจับความขัดแย้ง (conflict) ทำที่ระดับ `SurveyedBuilding` (aggregate) เท่านั้น** โดยเทียบ `data_version`/`last_modified_at`/`last_modified_by_device_id` ของอาคารแม่ ส่วน `client_generated_id` ของ entity ย่อย (`SurveyParticipant`, `SurroundingHazard`, `ExternalDamage`, `StructuralDamage`, `ComponentDamage`, `ElectricalSystemDamage`) มีไว้เพื่อ idempotent upsert และให้ `DamagePhoto.linked_area_id` อ้างอิงได้อย่างเสถียรเท่านั้น (ดูเหตุผลเต็มที่ [[db-spec#10. กฎทางธุรกิจที่กระทบโครงสร้างข้อมูล|db-spec หัวข้อ 10 ข้อ 2-3]]); **สำหรับไฟล์ (`DamagePhoto`/`Sketch`) ต้องส่งต่อจากจุดค้างเสมอ (NFR-02)** — ก่อนส่ง(ต่อ)เนื้อไฟล์ใดๆ เซิร์ฟเวอร์ต้องอ้างอิง `uploaded_bytes` ปัจจุบันของ `upload_session_id` นั้นก่อนเสมอ เพื่อให้อุปกรณ์ส่งเฉพาะส่วนที่ยังไม่ถึงเซิร์ฟเวอร์ ไม่ส่งซ้ำส่วนที่มีอยู่แล้ว; ถ้า `upload_session_id` ที่อุปกรณ์ส่งมาไม่ตรงกับที่เซิร์ฟเวอร์บันทึกไว้ล่าสุด หรือรอบเดิมค้างนานเกินเกณฑ์ที่ถือว่า stale เซิร์ฟเวอร์ต้องออก `upload_session_id` ใหม่และเริ่มนับ `uploaded_bytes` จาก 0 (ตามกฎทางธุรกิจข้อ 8 ใน [[db-spec#10. กฎทางธุรกิจที่กระทบโครงสร้างข้อมูล|db-spec หัวข้อ 10]]) — ขนาด chunk ที่แบ่งส่งจริงคือ **256 KB** ต่อครั้ง และเกณฑ์เวลา stale คือ **72 ชั่วโมง** นับจากเวลาที่เซิร์ฟเวอร์ได้รับส่วนไฟล์ล่าสุดสำเร็จ (ปิดแล้ว — ดูหัวข้อ 16 และ [[architecture#6.10 ขนาด chunk และเกณฑ์เวลา stale ของรอบอัปโหลดแบบ resumable (NFR-02)|architecture §6.10]])
- **กรณี error**: ไฟล์ภาพส่งไม่สำเร็จบางส่วน (สัญญาณขาดหาย) — ต้องส่งต่อได้จากตำแหน่ง `uploaded_bytes` ที่เซิร์ฟเวอร์ยืนยันล่าสุดของ `upload_session_id` เดิม โดยไม่ต้องเริ่มใหม่ทั้งหมด เว้นแต่ `upload_session_id` ไม่ตรงกันหรือรอบเดิม stale ซึ่งต้องเริ่มรอบใหม่ตามกฎทางธุรกิจข้างต้น, พบข้อมูลชุดเดียวกันถูกแก้ไขจากหลายอุปกรณ์ → ส่งต่อไปยัง 14.2, **อุปกรณ์ส่งคำขอซิงค์เกินโควตาที่กำหนด (rate limit ต่อ `device_id`)** → เซิร์ฟเวอร์ตอบสถานะ "ขอมากเกินไป" พร้อมระยะเวลาที่แนะนำให้ลองใหม่ (NFR-07 — กลไกปิดแล้ว ตัวเลขจริงยังไม่ปิด ดู [[#16. ประเด็นรอตัดสินใจ|หัวข้อ 16]])
...
- **กฎทางธุรกิจ**: กลยุทธ์ merge ที่ใช้จริง (ปิดแล้ว — ดูหัวข้อ 16 และ [[architecture#6.9 กลยุทธ์การจัดการความขัดแย้งของข้อมูล เมื่อ merge อัตโนมัติไม่ได้ (NFR-03)|architecture §6.9]]) คือ **ส่งให้มนุษย์ตัดสินใจทุกครั้งที่การเทียบเวอร์ชันระดับ aggregate พบว่าชนกัน** ไม่ทำ auto-merge ระดับฟิลด์ใดๆ ใน Phase นี้ — **Phase นี้ไม่มีการเปรียบเทียบค่าระดับฟิลด์เลย** การเทียบทำที่ระดับ `data_version`/`last_modified_at`/`last_modified_by_device_id` ของ `SurveyedBuilding` (aggregate) เท่านั้น (ดู [[db-spec#10. กฎทางธุรกิจที่กระทบโครงสร้างข้อมูล|db-spec หัวข้อ 10 ข้อ 3]]); "merge อัตโนมัติสำเร็จ" ในที่นี้หมายถึงเฉพาะกรณีที่การเทียบเวอร์ชันระดับ aggregate ไม่พบการชนกันเลย (เช่น อุปกรณ์อ้างอิง `data_version` ฐานตรงกับค่าปัจจุบันบนเซิร์ฟเวอร์ ไม่มีอุปกรณ์อื่นแก้ไขคั่นกลาง) เท่านั้น — **เมื่อพบว่าเวอร์ชันระดับ aggregate ชนกันจากหลายอุปกรณ์ ทุกกรณีต้องส่งให้มนุษย์ตัดสินใจเสมอ รวมถึงกรณีที่ทั้งสองอุปกรณ์บังเอิญแก้ไขคนละฟิลด์กัน หรือค่าที่แก้ไขบังเอิญตรงกัน ซึ่งในทางทฤษฎี merge อัตโนมัติได้** เพราะไม่มีกลไกเปรียบเทียบระดับฟิลด์ให้แยกแยะกรณีเหล่านี้ออกจากความขัดแย้งจริง — **นี่คือขอบเขตที่ตั้งใจของ Phase 1 ไม่ใช่การมองข้าม** (ผู้ใช้ยืนยันเมื่อ 2026-09-04) ผลคือ NFR-03 จึง**รองรับบางส่วน**ในเฟสนี้ (กลไกเปรียบเทียบระดับฟิลด์เป็นรายการเปิด ดู [[#16. ประเด็นรอตัดสินใจ|หัวข้อ 16]] และ [[architecture#6.9 กลยุทธ์การจัดการความขัดแย้งของข้อมูล เมื่อ merge อัตโนมัติไม่ได้ (NFR-03)|architecture §6.9]]); เมื่อ merge อัตโนมัติไม่ได้ (พบว่าเวอร์ชันระดับ aggregate ชนกัน) ต้องส่งต่อให้มนุษย์ตัดสินใจผ่าน [[#15. การแก้ไขความขัดแย้งของข้อมูลจากการซิงค์ (Manual Conflict Resolution)|หัวข้อ 15]] เสมอ (แก้ช่องว่างที่เคยรายงานไว้ใน `## NEEDS_NEW_REQUIREMENT` รอบก่อน ด้วย FR-30/FR-31); ทุกผลการซิงค์ (สำเร็จหรือขัดแย้ง) ต้องถูกแนบเข้า audit trail เสมอ (NFR-05, ดู [[architecture#5.5 Cross-cutting concerns|architecture §5.5]])
...
[[technology-stack]] มีเนื้อหาแล้ว (ตัดสินใจ 6 ชั้น: Field Client Framework, Local Persistent Store, Server-side Runtime, Primary Data Store, Media/Object Storage, **Protocol/Web Framework (REST over HTTP + Fastify — ปิดเพิ่มเมื่อ 2026-09-04)** — ดู [[technology-stack#3. การตัดสินใจรายชั้น|technology-stack หัวข้อ 3]]) **ประเด็น protocol/รูปแบบการสื่อสาร/web framework ปิดแล้ว** (เดิมรายการด้านล่างเคยเขียนไว้ว่า "ยังไม่ปิด" — ผู้ใช้เลือก REST over HTTP + Fastify แยกต่างหากจากการเลือก Server-side Runtime เมื่อ 2026-09-04) รายการด้านล่างนี้บางส่วนปิดแล้วตามการตัดสินใจนี้และการตัดสินใจอื่นในรอบเดียวกัน (ดูเครื่องหมาย "ปิดแล้ว" กำกับ) ส่วนที่เหลือยังรอการตัดสินใจเพิ่มเติม:

- รูปแบบการเรียก operation จริง (function call ภายใน monolith / message queue / API แบบใดก็ตาม) และ protocol การสื่อสาร — **ปิดแล้วเมื่อ 2026-09-04**: REST over HTTP + Fastify (ดู [[technology-stack#3. การตัดสินใจรายชั้น|technology-stack หัวข้อ 3]]) — **ส่วนที่ยังไม่ทำในรอบนี้**: การเพิ่ม HTTP method/path ระบุต่อ operation ในหัวข้อ 1-15 ของเอกสารนี้ (เช่น `POST /surveys`) แม้ตอนนี้จะรู้แล้วว่าเป็น REST — เจตนาเลื่อนออกไปเป็นรอบทำงานแยกต่างหาก เพราะกระทบทั้ง ~40 operation ในเอกสารนี้และ `detailed-design/` 17 ไฟล์พร้อมกัน การปนกับการปิดตัวเลข chunk/stale/merge ของรอบนี้จะทำให้ตรวจสอบความถูกต้องยากขึ้นและเพิ่มความเสี่ยงเอกสารไม่ sync ระหว่างทาง
- รูปแบบไฟล์ที่ใช้ส่งออกรายงาน (12.1) — ยังไม่ปิด
- กลยุทธ์ **auto-merge** ที่เป็นรูปธรรมก่อนตัดสินว่า "merge ไม่ได้" ใน 14.2 (last-write-wins บางส่วน / field-level merge / ส่งให้มนุษย์ตัดสินใจทั้งหมดทุกครั้ง) — **ปิดแล้วเมื่อ 2026-09-04**: เลือกส่งให้มนุษย์ตัดสินใจ**ทุกครั้ง**ที่พบความขัดแย้งจริง ไม่ทำ auto-merge ระดับฟิลด์ใดๆ ใน Phase นี้ (เหตุผลเต็มดู [[architecture#6.9 กลยุทธ์การจัดการความขัดแย้งของข้อมูล เมื่อ merge อัตโนมัติไม่ได้ (NFR-03)|architecture §6.9]]) — เส้นทาง manual resolution เมื่อ merge ไม่ได้ที่ออกแบบไว้ครบแล้วในหัวข้อ 15 จึงเป็นเส้นทางเดียวที่ใช้จริง
...
- วิธีจัดเก็บ/ส่งไฟล์ภาพขนาดใหญ่แบบแบ่งส่วน/ทำต่อได้ใน 14.1 และการจัดคิว/จำกัดอัตราจริงของ Server-side Sync Intake Queue (NFR-07) — **ตำแหน่งจัดเก็บปิดแล้ว** (Local filesystem บนเซิร์ฟเวอร์ ตาม [[technology-stack#3. การตัดสินใจรายชั้น|technology-stack หัวข้อ 3]]) และ**โมเดลข้อมูลระดับ logical ของสถานะการอัปโหลดที่ค้างไว้ปิดแล้วเช่นกัน — เพิ่มเข้ามา 2026-09-03** (`upload_session_id`/`total_file_size_bytes`/`uploaded_bytes`/`last_chunk_received_at` บน `DamagePhoto`/`Sketch` — ดู [[db-spec#6.1 DamagePhoto|db-spec 6.1]], [[db-spec#6.3 Sketch|db-spec 6.3]], [[db-spec#10. กฎทางธุรกิจที่กระทบโครงสร้างข้อมูล|db-spec หัวข้อ 10 ข้อ 8]]) **กลไก chunking จริง (ขนาด chunk ต่อครั้ง) และเกณฑ์เวลาที่ถือว่ารอบอัปโหลด stale ปิดแล้วเมื่อ 2026-09-04**: ขนาด chunk = **256 KB** ต่อครั้ง, เกณฑ์เวลา stale = **72 ชั่วโมง** นับจากเวลาที่ได้รับส่วนไฟล์ล่าสุดสำเร็จ (เหตุผลเต็มดู [[architecture#6.10 ขนาด chunk และเกณฑ์เวลา stale ของรอบอัปโหลดแบบ resumable (NFR-02)|architecture §6.10]]) — **กลไกจัดคิว/จำกัดอัตราของ Server-side Sync Intake Queue (NFR-07) ปิดแล้วเมื่อ 2026-09-04**: จำกัดโควตาแบบ token bucket ต่ออุปกรณ์ (`device_id`) — ไม่ใช่ต่อผู้ใช้ เพราะหนึ่งทีมอาจมีหลายเครื่อง — ร่วมกับจำกัดจำนวนตัวประมวลผล (worker) ที่ทำงานพร้อมกันฝั่งเซิร์ฟเวอร์ เมื่อเกินขีดจำกัดต้องตอบกลับสถานะ "ขอมากเกินไป" พร้อมระยะเวลาที่แนะนำให้ลองใหม่เสมอ (ดู [[architecture#6.12 กลไกจัดคิว/จำกัดอัตราคำขอซิงค์ต่ออุปกรณ์ (NFR-07)|architecture §6.12]] และ [[#14.1 ซิงค์ข้อมูลแบบสำรวจที่ค้างจากอุปกรณ์|14.1]]) — **ตัวเลขอัตราจริง (ขนาด token bucket, จำนวน worker สูงสุด, ระยะเวลาที่แนะนำให้ลองใหม่) ยังไม่ปิด ต้องจูนจาก load test ภายหลังที่มีสภาพแวดล้อมรันจริง — ห้ามเดาตัวเลข**; [[technology-stack]] ระบุไว้ชัดว่าการเลือก local filesystem แทน object storage ทำให้ resumable upload ตาม NFR-02 **ต้องเขียนเอง** ไม่ได้มาพร้อมใช้
- รูปแบบ/format จริงของ `SyncConflictVersion.snapshot_content` ใน 15.2 — **ปิดแล้วเมื่อ 2026-09-04**: เก็บเป็นข้อความรูปแบบ JSON ไว้ในคอลัมน์ข้อความยาว ไม่ใช้ชนิด JSON เฉพาะของฐานข้อมูล เพราะเนื้อหานี้มีไว้ให้มนุษย์เปรียบเทียบเท่านั้น ไม่เคยถูก query รายฟิลด์ (ดู [[db-spec#8.2 SyncConflictVersion|db-spec 8.2]] และ [[db-spec#11. ประเด็นรอตัดสินใจ|db-spec หัวข้อ 11]])
- กลไกเปรียบเทียบค่าระดับฟิลด์ (field-level comparison) เพื่อแยกกรณี "สองอุปกรณ์แก้คนละฟิลด์ / ค่าตรงกัน" (ในทางทฤษฎี merge อัตโนมัติได้) ออกจากกรณี "ฟิลด์เดียวกันขัดแย้งจริง" (ต้องให้มนุษย์ตัดสิน) ใน [[#14.2 ตรวจสอบ/แก้ไขความขัดแย้งของข้อมูลที่ซิงค์|14.2]] — **ยังไม่ทำในรอบนี้ เป็นขอบเขตที่ตั้งใจของ Phase 1 ไม่ใช่การมองข้าม**; ผู้ใช้ยืนยันเมื่อ 2026-09-04 ให้ยอมรับว่า NFR-03 รองรับ**บางส่วน**ในเฟสนี้แทนการเร่งสร้างกลไกนี้ก่อนมีข้อมูลจริงว่าจำเป็นหรือไม่ (ดู [[architecture#6.9 กลยุทธ์การจัดการความขัดแย้งของข้อมูล เมื่อ merge อัตโนมัติไม่ได้ (NFR-03)|architecture §6.9]]) — **เงื่อนไขที่จะกลับมาพิจารณาทำกลไกนี้**: มีข้อมูลจริงจากการใช้งาน Phase 1 ว่าคิวรอแก้ไขด้วยมือ (หัวข้อ 15) ยาวเกินกว่าหัวหน้าผู้สำรวจ/ผู้ดูแลระบบจะรับไหว หรือพบว่ากรณีสองอุปกรณ์แก้คนละฟิลด์ของอาคารเดียวกันพร้อมกันเกิดขึ้นบ่อยในทางปฏิบัติ — **ยังไม่ตัดสินกลไกหรือตัวเลขใดๆ ของเรื่องนี้**

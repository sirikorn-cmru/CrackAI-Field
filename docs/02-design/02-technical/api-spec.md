...
- **Input**: `username`, `credential_secret`, `device_id`, `cached_pin_secret` (ไม่บังคับสำหรับบทบาทอื่น — **จำเป็นเมื่อ** ผู้เรียกมี `role = ผู้สำรวจภาคสนาม` เพื่อตั้ง/เปลี่ยน PIN สำหรับปลดล็อกออฟไลน์ครั้งถัดไปบนอุปกรณ์นี้)
- **Output**: `Session` ใหม่ (`id`, `user_id`, `device_id`, `issued_at`, `expires_at`, `is_offline_cached = false`, `status = ใช้งานอยู่`, `failed_pin_attempt_count = 0`) พร้อมข้อมูล `User.role` เพื่อให้ client กำหนดเมนู/หน้าจอที่อนุญาต — ถ้าระบุ `cached_pin_secret` มาด้วย ให้บันทึกลง `Session.cached_pin_secret` เพื่อใช้ในการปลดล็อกออฟไลน์ครั้งถัดไปผ่าน [[#1.3 เข้าสู่ระบบด้วย credential ที่แคชไว้ขณะออฟไลน์|1.3]] (FR-25)
- **กฎทางธุรกิจ**: ตรวจสอบ `credential_secret` ตรงกับที่บันทึกไว้ของ `username`, บัญชีต้องมี `account_status = ใช้งานได้` เท่านั้นจึงเข้าสู่ระบบได้; ผู้สำรวจภาคสนามต้องตั้ง/ยืนยัน PIN ใหม่ทุกครั้งที่เข้าสู่ระบบด้วยรหัสผ่านสำเร็จขณะมีสัญญาณ (FR-25) — `cached_pin_secret` เก็บแยกจาก `credential_secret` เสมอ ทั้งสองไม่ปะปนกันในโครงสร้างข้อมูล (สอดคล้อง [[architecture#6.8 แยกพื้นที่จัดเก็บ credential/PIN ออกจากข้อมูลแบบสำรวจในเครื่อง (NFR-11)|architecture §6.8]]); การเข้าสู่ระบบสำเร็จรีเซ็ต `failed_pin_attempt_count` ของ `Session` บนอุปกรณ์นั้นเป็น 0 เสมอ (NFR-11)
...
- **Input**: `username`, `cached_pin_secret` (PIN ที่ผู้สำรวจกรอก — ตรวจสอบกับค่า `Session.cached_pin_secret` ที่แคชไว้บนอุปกรณ์เดียวกันเท่านั้น ไม่ติดต่อเซิร์ฟเวอร์เพราะไม่มีสัญญาณ), `device_id`
- **Output**:
  - **กรอกถูกต้อง**: ใช้งาน `Session` เดิมที่แคชไว้บนอุปกรณ์ (`is_offline_cached = true`) ต่อได้ทันที, `expires_at` เดิมยังใช้อ้างอิงตามอายุ credential ที่แคชไว้ (นับจากครั้งล่าสุดที่เชื่อมต่อเซิร์ฟเวอร์สำเร็จ — NFR-09), `Session.failed_pin_attempt_count` รีเซ็ตเป็น 0 (NFR-11)
  - **กรอกผิดแต่ยังไม่ครบจำนวนสูงสุด**: `Session.failed_pin_attempt_count` เพิ่มค่าอีก 1 (นับต่อเนื่องเฉพาะบนอุปกรณ์นี้ ไม่ซิงค์ขึ้นเซิร์ฟเวอร์ — NFR-11) ปฏิเสธการปลดล็อกครั้งนี้
  - **กรอกผิดครบจำนวนครั้งสูงสุดที่กำหนด (NFR-11)**: ล้าง `Session.cached_pin_secret` และ `Session.failed_pin_attempt_count` ทันทีในธุรกรรมเดียว ตั้ง `Session.status = ถูกเพิกถอน` — **ไม่แตะ/ไม่ลบ `SurveyedBuilding`, entity ย่อยของแบบสำรวจ หรือคิวรอซิงค์บนอุปกรณ์เดียวกันเลย** (NFR-01)
- **กฎทางธุรกิจ**: ใช้ได้เฉพาะเมื่อไม่มีสัญญาณอินเทอร์เน็ต, session ที่ได้ต้องมีอายุจำกัดตาม NFR-09 และบังคับต่ออายุ/ยืนยันตัวตนใหม่กับเซิร์ฟเวอร์ทันทีที่ครบกำหนดหรือกลับมามีสัญญาณ; ต้องนับจำนวนครั้งที่กรอก `cached_pin_secret` ผิดติดต่อกันบนอุปกรณ์นี้เท่านั้น (NFR-11) — จำนวนครั้งสูงสุดที่กำหนดคือ **5 ครั้งติดต่อกัน** (ยืนยันแล้วเมื่อ 2026-09-04 — ปิดประเด็นนี้ ดู [[#16. ประเด็นรอตัดสินใจ|หัวข้อ 16]]); หลังเกิด lockout ต้องเข้าสู่ระบบใหม่ทั้งหมดผ่าน [[#1.1 เข้าสู่ระบบ|1.1]] เท่านั้น ไม่มีทางปลดล็อกอื่นบนอุปกรณ์นี้อีก
- **กรณี error**: credential ที่แคชไว้หมดอายุแล้ว (ต้องเชื่อมต่อเซิร์ฟเวอร์เพื่อเข้าสู่ระบบใหม่), ไม่เคยเข้าสู่ระบบสำเร็จบนอุปกรณ์นี้มาก่อนจึงไม่มี credential ให้แคช, PIN ไม่ตรงกับที่แคชไว้ (นับเพิ่มใน `failed_pin_attempt_count`), กรอก PIN ผิดครบจำนวนครั้งสูงสุดที่กำหนด (ล็อกอุปกรณ์นี้ทันที บังคับเข้าสู่ระบบใหม่ด้วยรหัสผ่านผ่าน 1.1 เมื่อกลับมามีสัญญาณเท่านั้น — NFR-11)
- **FR/NFR**: [[feature-list#14. ยืนยันตัวตนและเข้าสู่ระบบ|FR-27, NFR-09, NFR-11]]
...
- **กรณี error**: บัญชีถูกปิดใช้งานระหว่างที่ออฟไลน์อยู่ (ต้องปฏิเสธและบังคับออกจากระบบ), `Session.status = ถูกเพิกถอน` อยู่แล้ว (เช่นจาก lockout ตาม NFR-11 ใน [[#1.3 เข้าสู่ระบบด้วย credential ที่แคชไว้ขณะออฟไลน์|1.3]]) — ต้องปฏิเสธและบังคับเข้าสู่ระบบใหม่ทั้งหมดผ่าน [[#1.1 เข้าสู่ระบบ|1.1]] เท่านั้น ไม่สามารถต่ออายุ session ที่ถูกเพิกถอนแล้วได้
...
- **Input**: รายการระเบียนที่ค้างซิงค์ทั้งหมดจากอุปกรณ์ (`SurveyedBuilding`, `SurroundingHazard`, `ExternalDamage`, `StructuralDamage`, `ComponentDamage`, `ElectricalSystemDamage`, `DamagePhoto`, `AIAnalysisResult`, `Sketch`, `SurveyParticipant`) ระบุด้วย `client_generated_id` ของแต่ละระเบียน, `device_id`; สำหรับระเบียนที่มีไฟล์ (`DamagePhoto`, `Sketch`) เพิ่มเติม: `total_file_size_bytes`, `upload_session_id` (ค่าที่อุปกรณ์ถืออยู่จากรอบก่อนหน้าถ้าเป็นการส่งต่อ — ไม่ระบุถ้าเป็นการเริ่มอัปโหลดไฟล์นี้ครั้งแรก) และเนื้อไฟล์ส่วนที่จะส่ง (เริ่มจากตำแหน่งที่เซิร์ฟเวอร์เคยยืนยันไว้)
- **Output**: สถานะการซิงค์ต่อระเบียน (`sync_status = ซิงค์สำเร็จ` พร้อม `id` ที่เซิร์ฟเวอร์กำหนดให้, หรือ `sync_status = มีความขัดแย้งรอแก้ไข`) — สำหรับ `DamagePhoto`/`Sketch` แต่ละรายการ เซิร์ฟเวอร์ต้องรายงานกลับเสมอว่า `upload_session_id` ปัจจุบันคือค่าใด (คงเดิมถ้าเป็นการส่งต่อ หรือค่าที่ออกใหม่ถ้าเริ่มรอบใหม่ตามกฎทางธุรกิจ) และ `uploaded_bytes` ล่าสุดที่ได้รับสำเร็จแล้ว เพื่อให้อุปกรณ์รู้ว่ารับไปถึงไหนก่อนส่งต่อในรอบถัดไป + `AuditTrailEntry` ใหม่ต่อ `SurveyedBuilding` ที่ซิงค์สำเร็จแต่ละอาคาร (`related_entity_name = SurveyedBuilding`, `action_type = ซิงค์ข้อมูลสำเร็จ`, `performed_by_user_id` ไม่บังคับ, `note` ระบุ `device_id` ต้นทาง) เพื่อ auditability ของผลการซิงค์ (NFR-05, ดู [[architecture#5.5 Cross-cutting concerns|architecture §5.5]])
- **กฎทางธุรกิจ**: ต้องรองรับไฟล์ภาพขนาดใหญ่แบบทำต่อได้เมื่อสัญญาณขาดหาย (NFR-02) และรองรับหลายอุปกรณ์/หลายทีมซิงค์พร้อมกันจำนวนมาก (NFR-07) โดยไม่บล็อกการทำงานอื่นของ Field Client — คำขอซิงค์เข้าคิวผ่าน [[architecture#2. Logical Component|คิวรับคำขอซิงค์ฝั่งเซิร์ฟเวอร์ (Server-side Sync Intake Queue)]] ก่อนประมวลผลเพื่อจัดลำดับ/จำกัดอัตราไม่ให้ระบบล่มเมื่อหลายทีมซิงค์พร้อมกันหลังเกิดเหตุ; ใช้ `client_generated_id` เป็น idempotency key กันบันทึกซ้ำสำหรับทุก entity ที่ระบุไว้ใน Input — **แต่การตรวจจับความขัดแย้ง (conflict) ทำที่ระดับ `SurveyedBuilding` (aggregate) เท่านั้น** โดยเทียบ `data_version`/`last_modified_at`/`last_modified_by_device_id` ของอาคารแม่ ส่วน `client_generated_id` ของ entity ย่อย (`SurveyParticipant`, `SurroundingHazard`, `ExternalDamage`, `StructuralDamage`, `ComponentDamage`, `ElectricalSystemDamage`) มีไว้เพื่อ idempotent upsert และให้ `DamagePhoto.linked_area_id` อ้างอิงได้อย่างเสถียรเท่านั้น (ดูเหตุผลเต็มที่ [[db-spec#10. กฎทางธุรกิจที่กระทบโครงสร้างข้อมูล|db-spec หัวข้อ 10 ข้อ 2-3]]); **สำหรับไฟล์ (`DamagePhoto`/`Sketch`) ต้องส่งต่อจากจุดค้างเสมอ (NFR-02)** — ก่อนส่ง(ต่อ)เนื้อไฟล์ใดๆ เซิร์ฟเวอร์ต้องอ้างอิง `uploaded_bytes` ปัจจุบันของ `upload_session_id` นั้นก่อนเสมอ เพื่อให้อุปกรณ์ส่งเฉพาะส่วนที่ยังไม่ถึงเซิร์ฟเวอร์ ไม่ส่งซ้ำส่วนที่มีอยู่แล้ว; ถ้า `upload_session_id` ที่อุปกรณ์ส่งมาไม่ตรงกับที่เซิร์ฟเวอร์บันทึกไว้ล่าสุด หรือรอบเดิมค้างนานเกินเกณฑ์ที่ถือว่า stale เซิร์ฟเวอร์ต้องออก `upload_session_id` ใหม่และเริ่มนับ `uploaded_bytes` จาก 0 (ตามกฎทางธุรกิจข้อ 8 ใน [[db-spec#10. กฎทางธุรกิจที่กระทบโครงสร้างข้อมูล|db-spec หัวข้อ 10]]) — ขนาด chunk ที่แบ่งส่งจริงและเกณฑ์เวลา stale เป็นประเด็นรอตัดสินใจ (ดูหัวข้อ 16) **ห้ามเดาตัวเลข**
- **กรณี error**: ไฟล์ภาพส่งไม่สำเร็จบางส่วน (สัญญาณขาดหาย) — ต้องส่งต่อได้จากตำแหน่ง `uploaded_bytes` ที่เซิร์ฟเวอร์ยืนยันล่าสุดของ `upload_session_id` เดิม โดยไม่ต้องเริ่มใหม่ทั้งหมด เว้นแต่ `upload_session_id` ไม่ตรงกันหรือรอบเดิม stale ซึ่งต้องเริ่มรอบใหม่ตามกฎทางธุรกิจข้างต้น, พบข้อมูลชุดเดียวกันถูกแก้ไขจากหลายอุปกรณ์ → ส่งต่อไปยัง 14.2
...
- วิธีจัดเก็บ/ส่งไฟล์ภาพขนาดใหญ่แบบแบ่งส่วน/ทำต่อได้ใน 14.1 และการจัดคิว/จำกัดอัตราจริงของ Server-side Sync Intake Queue (NFR-07) — **ตำแหน่งจัดเก็บปิดแล้ว** (Local filesystem บนเซิร์ฟเวอร์ ตาม [[technology-stack#3. การตัดสินใจรายชั้น|technology-stack หัวข้อ 3]]) และ**โมเดลข้อมูลระดับ logical ของสถานะการอัปโหลดที่ค้างไว้ปิดแล้วเช่นกัน — เพิ่มเข้ามา 2026-09-03** (`upload_session_id`/`total_file_size_bytes`/`uploaded_bytes`/`last_chunk_received_at` บน `DamagePhoto`/`Sketch` — ดู [[db-spec#6.1 DamagePhoto|db-spec 6.1]], [[db-spec#6.3 Sketch|db-spec 6.3]], [[db-spec#10. กฎทางธุรกิจที่กระทบโครงสร้างข้อมูล|db-spec หัวข้อ 10 ข้อ 8]]) แต่**กลไก chunking จริง (ขนาด chunk ต่อครั้ง), เกณฑ์เวลาที่ถือว่ารอบอัปโหลด stale, และการจัดคิวจริงยังไม่ปิด** — [[technology-stack]] ระบุไว้ชัดว่าการเลือก local filesystem แทน object storage ทำให้ resumable upload ตาม NFR-02 **ต้องเขียนเอง** ไม่ได้มาพร้อมใช้
- รูปแบบ/format จริงของ `SyncConflictVersion.snapshot_content` ใน 15.2 — ยังไม่ปิด
- **จำนวนครั้งสูงสุดที่ยอมให้กรอก `cached_pin_secret` ผิดติดต่อกันก่อน lockout ใน [[#1.3 เข้าสู่ระบบด้วย credential ที่แคชไว้ขณะออฟไลน์|1.3]] (NFR-11) — เพิ่มเข้ามา 2026-09-03 · ปิดแล้วเมื่อ 2026-09-04**: ผู้ใช้ยืนยันตัวเลขเป็น **5 ครั้งติดต่อกัน** แล้ว (เดิม spec เสนอไว้เป็นช่วง 5-10 ครั้ง) — ดู [[db-spec#11. ประเด็นรอตัดสินใจ|db-spec หัวข้อ 11]] ประกอบ — **NFR-11 ไม่ต้องการ server-side operation ใหม่**: กลไกนับ/ล้าง credential ทั้งหมดเกิดขึ้นบน Field Client App/Local Persistent Store ล้วนๆ ขณะออฟไลน์ (ดู [[architecture#6.8 แยกพื้นที่จัดเก็บ credential/PIN ออกจากข้อมูลแบบสำรวจในเครื่อง (NFR-11)|architecture §6.8]]) จึงฝังเป็นส่วนหนึ่งของกฎทางธุรกิจใน 1.3 ที่มีอยู่แล้วแทนการเพิ่ม operation ใหม่ ยังคงเป็นค่าที่**ตั้งค่าได้ (configurable)** ไม่ hardcode ตามที่ [[architecture#7. ข้อสมมติ|architecture §7 ข้อสมมติ 7]] ระบุไว้

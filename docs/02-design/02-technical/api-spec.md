...
- **กฎทางธุรกิจ**: ใช้ได้เฉพาะเมื่อไม่มีสัญญาณอินเทอร์เน็ต, session ที่ได้ต้องมีอายุจำกัดตาม NFR-09 และบังคับต่ออายุ/ยืนยันตัวตนใหม่กับเซิร์ฟเวอร์ทันทีที่ครบกำหนดหรือกลับมามีสัญญาณ; ต้องนับจำนวนครั้งที่กรอก `cached_pin_secret` ผิดติดต่อกันบนอุปกรณ์นี้เท่านั้น (NFR-11) — จำนวนครั้งสูงสุดที่กำหนดยังไม่ยืนยัน (ดู [[#16. ประเด็นรอตัดสินใจ|หัวข้อ 16]]) **ห้ามเดาตัวเลข**; หลังเกิด lockout ต้องเข้าสู่ระบบใหม่ทั้งหมดผ่าน [[#1.1 เข้าสู่ระบบ|1.1]] เท่านั้น ไม่มีทางปลดล็อกอื่นบนอุปกรณ์นี้อีก
...
- **จำนวนครั้งสูงสุดที่ยอมให้กรอก `cached_pin_secret` ผิดติดต่อกันก่อน lockout ใน [[#1.3 เข้าสู่ระบบด้วย credential ที่แคชไว้ขณะออฟไลน์|1.3]] (NFR-11) — เพิ่มเข้ามา 2026-09-03**: เป็นประเด็นนโยบาย/โดเมนที่ผู้ใช้ยังไม่ยืนยันตัวเลข (spec เสนอไว้เป็นช่วง 5-10 ครั้ง) ไม่ใช่ประเด็น tech stack — ดู [[db-spec#11. ประเด็นรอตัดสินใจ|db-spec หัวข้อ 11]] ประกอบ **ห้ามเดาค่าตัวเลขนี้** — ยังไม่ปิด — **NFR-11 ไม่ต้องการ server-side operation ใหม่**: กลไกนับ/ล้าง credential ทั้งหมดเกิดขึ้นบน Field Client App/Local Persistent Store ล้วนๆ ขณะออฟไลน์ (ดู [[architecture#6.8 แยกพื้นที่จัดเก็บ credential/PIN ออกจากข้อมูลแบบสำรวจในเครื่อง (NFR-11)|architecture §6.8]]) จึงฝังเป็นส่วนหนึ่งของกฎทางธุรกิจใน 1.3 ที่มีอยู่แล้วแทนการเพิ่ม operation ใหม่

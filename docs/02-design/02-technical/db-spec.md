...
| upload_session_id | ตัวระบุเฉพาะ | ไม่ (จำเป็นเมื่อ sync_status = กำลังซิงค์) | รหัสรอบการอัปโหลดไฟล์นี้ปัจจุบัน ใช้แยกรอบอัปโหลดที่กำลังส่งต่อจากรอบที่ต้องล้างแล้วเริ่มใหม่ (NFR-02 — เพิ่มเข้ามาเพื่อรองรับ resumable upload บน local filesystem ดูกฎทางธุรกิจข้อ 9 ในหัวข้อ 10) |
...
| last_chunk_received_at | วันที่-เวลา | ไม่ (จำเป็นเมื่อ sync_status = กำลังซิงค์) | เวลาที่เซิร์ฟเวอร์ได้รับส่วนของไฟล์นี้ล่าสุดสำเร็จ ใช้ตรวจจับรอบอัปโหลดที่ค้างนานเกินควร (stale) ตามกฎทางธุรกิจข้อ 9 ในหัวข้อ 10 |
...
- อัลกอริทึม/รูปแบบการสร้าง `client_generated_id` (เช่น UUID v4 หรือรูปแบบอื่น) และวิธีเข้ารหัส `credential_secret` — ยังไม่ปิด

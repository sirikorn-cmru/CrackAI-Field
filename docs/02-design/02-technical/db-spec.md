...
| archive_status | ค่าเลือกจากรายการ (ปกติ / เก็บถาวร) | ใช่ | สถานะการเก็บถาวรของระเบียนนี้ — **เพิ่มเข้ามา 2026-09-04**: เตรียมไว้รองรับนโยบายเก็บรักษา/archive ระยะยาว (เก็บถาวรไม่ลบอัตโนมัติ ปิดแล้ว — ดูหัวข้อ 11) แต่**ยังไม่เปิดใช้งานจริงในเฟสนี้** ทุกระเบียนมีค่าเริ่มต้นเป็น "ปกติ" เสมอ ไม่มี operation ใดใน [[api-spec]] อ่าน/เปลี่ยนค่านี้จนกว่าจะมีระยะเวลาเก็บรักษาที่ชัดเจนจากหน่วยงาน (ยังไม่ปิด ดูหัวข้อ 11) |

**ดัชนีที่ออกแบบไว้ (เชิงตรรกะ — ปิดแล้วเมื่อ 2026-09-04)**: เพื่อรองรับตัวกรองของ FR-32/FR-33 ให้ค้นหาได้อย่างมีประสิทธิภาพ ออกแบบ index (ระดับ logical เท่านั้น ไม่ระบุการ implement จริงของ engine) ไว้ 3 ชุด: (ก) `(related_surveyedbuilding_id, performed_at)` รองรับ FR-32/FR-33 กรองตามอาคาร/ทีมพร้อมเรียงตามเวลา (ข) `(action_type, performed_at)` รองรับตัวกรอง "ประเภทการกระทำ" ของ FR-33 (ค) `(performed_by_user_id, performed_at)` — **เพิ่มเข้ามา 2026-09-04** รองรับตัวกรอง "ผู้แก้ไข" ของ FR-33 (ดู [[api-spec#15.5.2 ค้นหา/กรองประวัติการแก้ไขทั่วทั้งระบบ|api-spec 15.5.2]]) — ทั้งสามชุดปิดแล้ว ส่วนกลยุทธ์รองรับปริมาณข้อมูลจำนวนมากในระยะยาวเมื่อไม่มีการลบอัตโนมัติ (เช่น partition/read replica) ยังไม่ปิด ดูหัวข้อ 11
...
| snapshot_content | ข้อความ | ใช่ | ค่าที่บันทึกของอาคารและระเบียนย่อยที่เกี่ยวข้อง ณ เวลานั้น — **ปิดแล้วเมื่อ 2026-09-04**: เก็บเป็นข้อความรูปแบบ JSON ไว้ในคอลัมน์ข้อความยาว ไม่ใช้ชนิด JSON เฉพาะของฐานข้อมูล เพราะเนื้อหานี้มีไว้ให้มนุษย์เปรียบเทียบเทียบเท่านั้น ไม่เคยถูก query รายฟิลด์ (ดูหัวข้อ 11) |
...
8. **การทำต่อได้ของการอัปโหลดไฟล์ขนาดใหญ่ (Resumable upload, NFR-02) — เพิ่มเข้ามา 2026-09-03 · ตัวเลขปิดแล้วเมื่อ 2026-09-04**: `DamagePhoto` และ `Sketch` แต่ละระเบียนมี `upload_session_id` / `total_file_size_bytes` / `uploaded_bytes` / `last_chunk_received_at` เพื่อให้ระบบตอบได้ว่าไฟล์นี้ส่งสำเร็จไปเท่าไรแล้วและอ้างอิงรอบการอัปโหลดใด — จำเป็นเพราะ [[technology-stack#3. การตัดสินใจรายชั้น|technology-stack หัวข้อ 3]] เลือก **Local filesystem บนเซิร์ฟเวอร์** เป็น Media/Object Storage ซึ่งไม่มีความสามารถ resumable upload มาให้พร้อมใช้เหมือน object storage จึงต้องออกแบบไว้ในโมเดลข้อมูลเอง อุปกรณ์ส่งเนื้อไฟล์เป็นส่วนย่อยขนาด **256 KB** ต่อครั้ง (chunk — ปิดแล้ว ผู้ใช้ยืนยันแล้ว ดูหัวข้อ 11 และ [[architecture#6.10 ขนาด chunk และเกณฑ์เวลา stale ของรอบอัปโหลดแบบ resumable (NFR-02)|architecture §6.10]] — ขนาด chunk เป็นรายละเอียดของ operation contract ใน [[api-spec#14.1 ซิงค์ข้อมูลแบบสำรวจที่ค้างจากอุปกรณ์|api-spec 14.1]] ไม่ใช่ attribute ที่เก็บไว้ในโมเดลนี้) เมื่ออุปกรณ์กลับมาส่งไฟล์ต่อ ต้องอ้างอิง `upload_session_id` เดิมและส่งต่อจากตำแหน่ง `uploaded_bytes` ที่เซิร์ฟเวอร์ยืนยันไว้เท่านั้น **ห้าม**เริ่มนับใหม่จาก 0 เว้นแต่เข้าเงื่อนไขใดเงื่อนไขหนึ่งต่อไปนี้: (ก) `upload_session_id` ที่อุปกรณ์ถืออยู่ไม่ตรงกับที่เซิร์ฟเวอร์บันทึกไว้ล่าสุดสำหรับ `client_generated_id` นั้น (เช่น แอป/ที่เก็บข้อมูลบนอุปกรณ์ถูกล้าง) หรือ (ข) `last_chunk_received_at` เก่ากว่าเกณฑ์เวลาที่ถือว่าค้างนานเกินควร (stale — **ปิดแล้ว: 72 ชั่วโมง** ผู้ใช้ยืนยันแล้ว ดูหัวข้อ 11) — ทั้งสองกรณีต้องล้าง `upload_session_id` / `uploaded_bytes` / `last_chunk_received_at` แล้วออก `upload_session_id` ใหม่เริ่มนับจาก 0 ก่อนรับไฟล์ต่อ; ตั้ง `sync_status = ซิงค์สำเร็จ` ได้ก็ต่อเมื่อ `uploaded_bytes = total_file_size_bytes` และบันทึกไฟล์สมบูรณ์แล้วเท่านั้น (ดู [[api-spec#14.1 ซิงค์ข้อมูลแบบสำรวจที่ค้างจากอุปกรณ์|api-spec 14.1]])
...
10. **จำกัดจำนวนครั้งที่กรอก PIN ผิดขณะปลดล็อกออฟไลน์ (NFR-11) — เพิ่มเข้ามา 2026-09-03 · ตัวเลขปิดแล้วเมื่อ 2026-09-04**: `Session.failed_pin_attempt_count` นับเฉพาะบนอุปกรณ์ที่ `is_offline_cached = true` เท่านั้น เพิ่มค่าทุกครั้งที่กรอก `cached_pin_secret` ผิดผ่าน [[api-spec#1.3 เข้าสู่ระบบด้วย credential ที่แคชไว้ขณะออฟไลน์|api-spec 1.3]] และรีเซ็ตเป็น 0 ทันทีที่ปลดล็อกสำเร็จหรือเข้าสู่ระบบด้วยรหัสผ่านสำเร็จใหม่ผ่าน [[api-spec#1.1 เข้าสู่ระบบ|1.1]] — เมื่อค่าครบ **5 ครั้งติดต่อกัน** (ยืนยันแล้ว ดูหัวข้อ 11) ต้องล้าง `cached_pin_secret`/`failed_pin_attempt_count` ของ `Session` นั้นทันทีในธุรกรรมเดียว พร้อมตั้ง `status = ถูกเพิกถอน` — **ห้ามการล้างนี้กระทบ `SurveyedBuilding`/entity ย่อยของแบบสำรวจหรือคิวรอซิงค์บนอุปกรณ์เดียวกันเด็ดขาด** (NFR-01) เพราะพื้นที่จัดเก็บ credential/PIN/session แยกจากพื้นที่จัดเก็บข้อมูลแบบสำรวจตั้งแต่ระดับโครงสร้าง ไม่ใช่พึ่งพา logic คัดกรองฟิลด์ (ดู [[architecture#6.8 แยกพื้นที่จัดเก็บ credential/PIN ออกจากข้อมูลแบบสำรวจในเครื่อง (NFR-11)|architecture §6.8]]); ค่า `failed_pin_attempt_count` ไม่ถูกส่ง/ซิงค์ขึ้นเซิร์ฟเวอร์เลย

## 11. ประเด็นรอตัดสินใจ

[[technology-stack]] มีเนื้อหาแล้ว (ตัดสินใจ 6 ชั้น รวม Primary Data Store = MySQL/MariaDB และ Protocol/Web Framework = REST over HTTP + Fastify (ปิดเพิ่มเมื่อ 2026-09-04) — ดู [[technology-stack#3. การตัดสินใจรายชั้น|technology-stack หัวข้อ 3]]) **ประเด็น "เป็น SQL หรือ NoSQL" ปิดแล้ว** (relational ยืนยันแล้ว) รายการด้านล่างนี้เป็นการตัดสินใจระดับ implementation ที่ละเอียดกว่านั้น **ปิดแล้วบางส่วนเมื่อ 2026-09-04** (ดูเครื่องหมาย "ปิดแล้ว" กำกับต่อรายการที่ปิดแล้ว) ส่วนที่เหลือยังรอการตัดสินใจเพิ่มเติม:
...
- **จำนวนครั้งสูงสุดที่ยอมให้กรอก `Session.cached_pin_secret` ผิดติดต่อกันก่อนล้าง credential/PIN (NFR-11) — เพิ่มเข้ามา 2026-09-03 · ปิดแล้วเมื่อ 2026-09-04**: ผู้ใช้ยืนยันตัวเลขเป็น **5 ครั้งติดต่อกัน** แล้ว (เดิม spec ต้นทางเสนอไว้เป็นช่วง 5-10 ครั้ง — [[20260828-01-flood-damage-survey#6. ข้อสมมติ / ประเด็นค้างพิจารณา|ข้อสมมติข้อ 11]]) — เป็นประเด็นนโยบาย/โดเมน ไม่ใช่ประเด็น tech stack ตัวเลขนี้ถูกฝังไว้ในกฎทางธุรกิจแล้ว (ดูหัวข้อ 10 ข้อ 10) โดยยังคงเป็นค่าที่**ตั้งค่าได้ (configurable)** ไม่ hardcode ตามที่ [[architecture#7. ข้อสมมติ|architecture §7 ข้อสมมติ 7]] ระบุไว้
- กลไก **merge อัตโนมัติ** ก่อนจะถือว่า "merge ไม่ได้" และส่งต่อให้มนุษย์แก้ไขผ่าน FR-30/FR-31 — **ปิดแล้วเมื่อ 2026-09-04**: ผู้ใช้ยืนยันให้ส่งต่อให้มนุษย์ตัดสินใจ**ทุกครั้ง**ที่พบความขัดแย้งจริง ไม่ทำ auto-merge ระดับฟิลด์ใดๆ ใน Phase นี้ (ทางเลือก last-write-wins บางส่วน/field-level merge ที่เคยพิจารณาไว้ถูกตัดออกในรอบนี้ — เก็บเป็นช่องทางเพิ่มเติมในอนาคตเมื่อมีข้อมูลปริมาณความขัดแย้งจริงจาก Phase 1 แล้ว เหตุผลเต็มดู [[architecture#6.9 กลยุทธ์การจัดการความขัดแย้งของข้อมูล เมื่อ merge อัตโนมัติไม่ได้ (NFR-03)|architecture §6.9]]) — เส้นทาง manual resolution ที่โมเดลข้อมูลนี้ออกแบบไว้แล้ว (หัวข้อ 8) จึงเป็นเส้นทางเดียวที่ใช้จริงเมื่อพบความขัดแย้ง ไม่มีขั้นตอน auto-merge ระดับฟิลด์คั่นกลางก่อนหน้านั้น
- **รูปแบบ/format จริงของ `SyncConflictVersion.snapshot_content` — ปิดแล้วเมื่อ 2026-09-04**: ผู้ใช้ยืนยันให้เก็บเป็นข้อความรูปแบบ JSON ในคอลัมน์ข้อความยาว (ไม่ใช้ชนิด JSON เฉพาะของฐานข้อมูล) เพราะเนื้อหานี้มีไว้ให้มนุษย์เปรียบเทียบเทียบเท่านั้น ไม่เคยถูก query รายฟิลด์ การใช้ชนิด JSON ของฐานข้อมูลจึงเพิ่มภาระ validation โดยไม่ได้ประโยชน์ (ดูหัวข้อ 8.2)
...
- **เกณฑ์เวลาที่ถือว่ารอบการอัปโหลดไฟล์ค้างนานเกินควร (stale) สำหรับ `DamagePhoto.last_chunk_received_at`/`Sketch.last_chunk_received_at` และขนาด chunk ที่แบ่งส่งจริงของ resumable upload — เพิ่มเข้ามา 2026-09-03 · ปิดแล้วเมื่อ 2026-09-04**: โครงสร้างข้อมูลรองรับการทำต่อได้ของการอัปโหลดแล้ว (`upload_session_id`/`total_file_size_bytes`/`uploaded_bytes`/`last_chunk_received_at` — ดูกฎทางธุรกิจข้อ 8 ในหัวข้อ 10) ผู้ใช้ยืนยันตัวเลขแล้ว: ขนาด chunk = **256 KB** ต่อครั้ง, เกณฑ์เวลา stale = **72 ชั่วโมง** นับจาก `last_chunk_received_at` ล่าสุด (เหตุผลเต็มดู [[architecture#6.10 ขนาด chunk และเกณฑ์เวลา stale ของรอบอัปโหลดแบบ resumable (NFR-02)|architecture §6.10]]) — ทั้งสองค่านี้เป็นการตัดสินใจระดับ implementation ที่ปิดร่วมกันระหว่างเอกสารนี้ (หัวข้อ 10 กฎข้อ 8) กับ [[api-spec#16. ประเด็นรอตัดสินใจ|api-spec หัวข้อ 16]] และ [[api-spec#14.1 ซิงค์ข้อมูลแบบสำรวจที่ค้างจากอุปกรณ์|api-spec 14.1]] แล้ว
- **นโยบายเก็บรักษา/archive ข้อมูล `AuditTrailEntry` ระยะยาว (retention policy) — เพิ่มเข้ามา 2026-09-02 · ปิดบางส่วนแล้วเมื่อ 2026-09-04**: ผู้ใช้ยืนยันนโยบายหลักแล้ว: **เก็บถาวรไม่ลบอัตโนมัติ แต่เตรียมกลไก archive ไว้ล่วงหน้าโดยยังไม่เปิดใช้งาน** ([[architecture#6.13 นโยบายเก็บรักษาและเตรียมกลไก archive ของ audit trail (NFR-05)|architecture §6.13]]) — เพิ่ม attribute `archive_status` ให้ `AuditTrailEntry` แล้ว (ดูหัวข้อ 7.2) ทุกระเบียนคงค่า "ปกติ" ไปก่อน ไม่มี operation ใดอ่าน/เขียนค่านี้ในเฟสนี้ — **ระยะเวลาเก็บรักษาก่อนเปิดใช้กลไก archive จริงยังไม่ปิด** รอหน่วยงาน (กรมโยธาธิการและผังเมือง) กำหนดตามระเบียบงานสารบรรณ **ห้ามเดาตัวเลข**
- กลยุทธ์การทำดัชนี/ค้นหาที่มีประสิทธิภาพสำหรับ `AuditTrailEntry` เมื่อข้อมูลสะสมมากขึ้นเรื่อยๆ โดยไม่มีการลบอัตโนมัติ (รองรับการค้นหา/กรองของ FR-33) — **ปิดบางส่วนแล้วเมื่อ 2026-09-04**: ชุด index เชิงตรรกะทั้ง 3 ชุดที่ตัวกรองของ FR-32/FR-33 ต้องใช้ปิดแล้ว (`(related_surveyedbuilding_id, performed_at)`, `(action_type, performed_at)`, `(performed_by_user_id, performed_at)` — ดูหมายเหตุท้ายตาราง 7.2) **ยังไม่ปิด**: กลยุทธ์รองรับปริมาณข้อมูลจำนวนมากในระยะยาว (เช่น partition/read replica) เมื่อรวมกับนโยบาย archive ที่เตรียมไว้แต่ยังไม่เปิดใช้ (ดูรายการก่อนหน้า) — [[technology-stack#6. เกณฑ์ที่จะทำให้ต้องทบทวนการตัดสินใจนี้ใหม่|technology-stack หัวข้อ 6 ข้อ 6]] และ [[architecture#8. ประเด็นรอตัดสินใจ|architecture §8]] เปิดประเด็นนี้ไว้ตรงกัน

...
3. **ขอบเขตการตรวจจับ/แก้ไขความขัดแย้งอยู่ที่ระดับ `SurveyedBuilding` (aggregate) เท่านั้น ไม่ใช่รายแถวของ entity ย่อย และ Phase 1 ไม่มีการเปรียบเทียบค่าระดับฟิลด์เลย (NFR-03, NFR-07)**: `SurveyedBuilding` มี `data_version`, `last_modified_at`, `last_modified_by_device_id` ให้ Sync Service เทียบเวอร์ชันจากหลายอุปกรณ์ก่อนตัดสินใจ merge/แจ้งเตือน — การแก้ไข entity ย่อยใดๆ ก็ตามจะทำให้ `data_version` ของ `SurveyedBuilding` แม่เพิ่มขึ้นเสมอ เมื่อการเทียบเวอร์ชันระดับ aggregate พบว่าชนกันจากหลายอุปกรณ์ ระบบตั้ง `sync_status = มีความขัดแย้งรอแก้ไข` และสร้าง `SyncConflict` + `SyncConflictVersion` (ดูหัวข้อ 8) เก็บทุกเวอร์ชันที่ส่งเข้ามาไว้เปรียบเทียบ จนกว่าหัวหน้าผู้สำรวจ/ผู้ดูแลระบบจะแก้ไขด้วยมือผ่าน FR-30/FR-31 เสมอ — **รวมถึงกรณีที่สองอุปกรณ์บังเอิญแก้ไขคนละฟิลด์กัน หรือค่าที่แก้ไขบังเอิญตรงกัน ซึ่งในทางทฤษฎี merge อัตโนมัติได้** เพราะโมเดลข้อมูลนี้ไม่มีกลไกเปรียบเทียบระดับฟิลด์ให้แยกกรณีดังกล่าวออกจากความขัดแย้งจริง (field เดียวกันถูกแก้ไขต่างค่ากัน) — **นี่คือขอบเขตที่ตั้งใจของ Phase 1 ไม่ใช่การมองข้าม** (ผู้ใช้ยืนยันเมื่อ 2026-09-04) ผลคือ NFR-03 จึง**รองรับบางส่วน**ในเฟสนี้ (กลไกเปรียบเทียบระดับฟิลด์เป็นรายการเปิด ดูหัวข้อ 11 และ [[architecture#6.9 กลยุทธ์การจัดการความขัดแย้งของข้อมูล เมื่อ merge อัตโนมัติไม่ได้ (NFR-03)|architecture §6.9]])
...
- **กลไกเปรียบเทียบค่าระดับฟิลด์ (field-level comparison) เพื่อปิดช่องว่างของ NFR-03 ให้ครบ** — เพื่อแยกกรณี "สองอุปกรณ์แก้คนละฟิลด์ / ค่าตรงกัน" (ในทางทฤษฎี merge อัตโนมัติได้) ออกจากกรณี "ฟิลด์เดียวกันขัดแย้งจริง" (ต้องให้มนุษย์ตัดสิน) — **ยังไม่ทำในรอบนี้ เป็นขอบเขตที่ตั้งใจของ Phase 1 ไม่ใช่การมองข้าม**; ผู้ใช้ยืนยันเมื่อ 2026-09-04 ให้ยอมรับว่า NFR-03 รองรับ**บางส่วน**ในเฟสนี้ (ดูหัวข้อ 10 ข้อ 3 และ [[architecture#6.9 กลยุทธ์การจัดการความขัดแย้งของข้อมูล เมื่อ merge อัตโนมัติไม่ได้ (NFR-03)|architecture §6.9]]) — **เงื่อนไขที่จะกลับมาพิจารณาทำกลไกนี้**: มีข้อมูลจริงจากการใช้งาน Phase 1 ว่าคิวรอแก้ไขด้วยมือ ([[api-spec#15. การแก้ไขความขัดแย้งของข้อมูลจากการซิงค์ (Manual Conflict Resolution)|api-spec หัวข้อ 15]]) ยาวเกินกว่าหัวหน้าผู้สำรวจ/ผู้ดูแลระบบจะรับไหว หรือพบว่ากรณีสองอุปกรณ์แก้คนละฟิลด์ของอาคารเดียวกันพร้อมกันเกิดขึ้นบ่อยในทางปฏิบัติ — **ยังไม่ตัดสินกลไกหรือตัวเลขใดๆ ของเรื่องนี้**

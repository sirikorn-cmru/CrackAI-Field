...
กลยุทธ์ **auto-merge** ที่เป็นรูปธรรม (ก่อนตัดสินว่า "merge ไม่ได้" ต้องส่งต่อ 15) **ปิดแล้วเมื่อ 2026-09-04**: ไม่ทำ auto-merge ระดับฟิลด์ใดๆ ใน Phase นี้ — ส่งให้มนุษย์ตัดสินใจทุกครั้งที่พบความขัดแย้งจริง คำว่า "merge อัตโนมัติสำเร็จ" ในไดอะแกรม §1/§3 ข้างต้นหมายถึงเฉพาะกรณีที่ไม่มีความขัดแย้งจริงเกิดขึ้นเลย (เช่น แก้ไขคนละฟิลด์ หรือค่าตรงกันพอดี) เท่านั้น ไม่ใช่การ merge ระดับฟิลด์จริงแต่อย่างใด ดู [[api-spec#16. ประเด็นรอตัดสินใจ|api-spec หัวข้อ 16]], [[architecture#6.9 กลยุทธ์การจัดการความขัดแย้งของข้อมูล เมื่อ merge อัตโนมัติไม่ได้ (NFR-03)|architecture §6.9]]

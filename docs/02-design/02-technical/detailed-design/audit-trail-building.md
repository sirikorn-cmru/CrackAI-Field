...
- **Retention policy ของ `AuditTrailEntry`**: นโยบายหลัก**ปิดแล้วเมื่อ 2026-09-04** — เก็บถาวรไม่ลบอัตโนมัติ พร้อมเพิ่ม attribute `archive_status` (ค่าเริ่มต้น "ปกติ") เตรียมรองรับกลไก archive ในอนาคต แต่**ยังไม่เปิดใช้งานจริงในเฟสนี้** (หน้านี้ไม่มี operation ใดอ่าน/เขียนค่านี้) — **ยังไม่ปิด**: ระยะเวลาเก็บรักษาก่อนเปิดใช้กลไก archive จริง รอหน่วยงาน (กรมโยธาธิการและผังเมือง) กำหนดตามระเบียบงานสารบรรณ ดู [[db-spec#7.2 AuditTrailEntry|db-spec §7.2]], [[db-spec#11. ประเด็นรอตัดสินใจ|db-spec §11]]
- กลยุทธ์การทำดัชนี/ค้นหาที่มีประสิทธิภาพเมื่อข้อมูล `AuditTrailEntry` สะสมมากขึ้น: ชุด index เชิงตรรกะ 3 ชุดที่รองรับตัวกรองของ FR-32/FR-33 **ปิดแล้วเมื่อ 2026-09-04** (`(related_surveyedbuilding_id, performed_at)`, `(action_type, performed_at)`, `(performed_by_user_id, performed_at)` — ดู [[db-spec#7.2 AuditTrailEntry|db-spec §7.2]]) — **ยังไม่ปิด**: กลยุทธ์รองรับปริมาณข้อมูลจำนวนมากในระยะยาวเมื่อไม่มีการลบอัตโนมัติ (เช่น partition/read replica) เป็นการตัดสินใจเชิงเทคนิคที่ต้องรอ `technology-stack.md` (ดู [[architecture#8. ประเด็นรอตัดสินใจ|architecture §8]])

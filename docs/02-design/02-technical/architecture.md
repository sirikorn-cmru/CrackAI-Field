...
|---|---|---|
| ผู้สำรวจภาคสนามลงพื้นที่สำรวจอาคาร | §4.1 | ครอบคลุมเต็ม journey เดียวจบในหนึ่ง diagram (เข้าสู่ระบบออฟไลน์ → กรอกข้อมูล → AI/กรอกเอง → สรุปผล → ซิงค์) |
...
- **การยืนยันตัวตนและบังคับสิทธิ์ (FR-25, FR-26, FR-27, NFR-08, NFR-09)** — เป็นความรับผิดชอบร่วมของ **Field Client App** (ตรวจสอบอายุ session ที่แคชไว้ในเครื่อง, บังคับเข้าสู่ระบบใหม่เมื่อครบกำหนด/กลับมามีสัญญาณ) และ **Backend Service** (ออก session จริง, ตรวจสอบสิทธิ์ RBAC ทุก request) — **Management Web Client** เรียกใช้ผลจาก Backend Service เสมอ ไม่ตัดสินใจสิทธิ์เอง
...
- **ความทนทานของข้อมูลขณะออฟไลน์ (NFR-01, NFR-02)** — เป็นความรับผิดชอบร่วมของ **ที่เก็บข้อมูล/ไฟล์ในเครื่อง**, **คิวรอซิงค์ในเครื่อง** (ทั้งสองภายใน Field Client App) และ **คิวรับคำขอฝั่งเซิร์ฟเวอร์** (ภายใน Sync & Conflict Resolution Service) ที่ต้องประสานกันให้ไม่มีข้อมูลสูญหายตลอดเส้นทางจากอุปกรณ์ถึงเซิร์ฟเวอร์
...
- **Runtime ของโมเดล AI วิเคราะห์รอยร้าว + รูปแบบไฟล์โมเดล** (บล็อกเฉพาะ Phase 3 เพราะยังไม่มีโมเดลจริง ไม่บล็อก Phase 1-2) — ดู [[technology-stack#4.1 Runtime ของโมเดล AI วิเคราะห์รอยร้าว + รูปแบบไฟล์โมเดล|technology-stack §4.1]] และดู [[#6.1 ตำแหน่งการทำงานของ AI วิเคราะห์รอยร้าว|§6.1]] ของเอกสารนี้ประกอบ เพราะ Field Client Framework ที่เลือกไว้มีข้อจำกัดที่ต้องพิจารณาซ้ำเมื่อถึงเวลาตัดสิน
- **Library เฉพาะตัวสำหรับ SQLite บน React Native** (เลือกไว้เพียงระดับ "SQLite" ยังไม่เลือก library ตัวใดตัวหนึ่ง) — ดู [[technology-stack#4.3 Library เฉพาะตัวสำหรับ SQLite บน React Native|technology-stack §4.3]] บล็อกการเริ่มเขียนโค้ดจริงของ Local Persistent Store แต่ไม่บล็อกงานออกแบบระดับ logical
- **กลไกยืนยันตัวตน/ออก session ที่เป็นรูปธรรม** (รวมถึงรูปแบบการเก็บ credential ที่แคชไว้บนอุปกรณ์อย่างปลอดภัยตาม NFR-09) — Field Client Framework และ Server-side Runtime ถูกเลือกแล้ว แต่กลไก/library เฉพาะสำหรับ auth และ secure storage ยังไม่ตัดสินใจ

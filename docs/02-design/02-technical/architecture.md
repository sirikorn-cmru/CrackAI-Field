...
> **หมายเหตุสำคัญ (ปรับปรุง 2026-09-03)**: `technology-stack.md` มีเนื้อหาแล้ว (ดู [[technology-stack]]) — ตัดสินใจแล้ว 5 ชั้น (Field Client Framework, Local Persistent Store, Server-side Runtime, Primary Data Store, Media/Object Storage) เอกสารนี้ยังคง**จงใจ**ใช้ชื่อ **component เชิงตรรกะ** เป็นหลักต่อไป — ไม่ใช่เพราะยังไม่มีการตัดสินใจ tech stack อีกต่อไป แต่เพราะสถาปัตยกรรมระดับนี้ควรอ่านเข้าใจและใช้งานได้ต่อแม้ stack จะเปลี่ยนในอนาคต ชื่อ stack จริงจะถูกอ้างอิงเฉพาะจุดที่การรู้ชื่อจริงเปลี่ยนความเข้าใจของผู้อ่านอย่างมีนัยสำคัญเท่านั้น (ดูรายละเอียดการตัดสินใจแต่ละชั้นและเหตุผลที่ [[technology-stack]]) ส่วนที่ยังไม่ตัดสินใจ (เช่น runtime ของโมเดล AI, hosting/deployment target) ดูที่ [[#8. ประเด็นรอตัดสินใจ|§8 ประเด็นรอตัดสินใจ]]
...
- **การควบคุมสถานะข้อมูลขัดแย้งไม่ให้ไหลเข้ารายงาน (NFR-10, FR-30, FR-31)** — เป็นความรับผิดชอบร่วมของ **Sync & Conflict Resolution Service** (ตั้งสถานะ "รอแก้ไขด้วยมือ" ตั้งแต่ต้นทาง), **Primary Data Store** (เก็บสถานะและทุกเวอร์ชันที่ขัดแย้ง), **Backend Service** (กรองสถานะนี้ออกจากทุกช่องทางอ่านข้อมูล) และ **Management Web Client** (แสดง/รับผลการแก้ไขจากมนุษย์)
...
2. **ระยะเวลาหมดอายุของ credential/session ที่แคชไว้ยังไม่กำหนดตัวเลข (spec ข้อสมมติ 6, NFR-09)** — สถาปัตยกรรมนี้สมมติว่า Field Client App ต้องมีกลไก**อายุ session ที่ตั้งค่าได้ (configurable)** ไม่ hardcode ค่าคงที่ ถ้าสมมติผิด (เช่นต้องมี policy ซับซ้อนกว่านี้ตามบทบาท/พื้นที่เสี่ยง) จะกระทบการออกแบบที่เก็บข้อมูล/ไฟล์ในเครื่องที่ต้องเก็บ metadata อายุ session เพิ่มเติม และอาจต้องเพิ่ม logic เฉพาะใน Field Client App
...
4. **จำนวนภาพสูงสุดต่อบริเวณและขนาด/ความละเอียดภาพยังไม่ระบุ (spec ข้อสมมติ 5, FR-12/NFR-02)** — สถาปัตยกรรมนี้สมมติว่าปริมาณอยู่ในระดับที่ที่เก็บข้อมูล/ไฟล์ในเครื่องของอุปกรณ์พกพาทั่วไปรองรับได้โดยไม่ต้องมีที่เก็บข้อมูลเสริมภายนอกที่หน้างาน ถ้าสมมติผิด (เช่นต้องรองรับภาพความละเอียดสูงจำนวนมากต่ออาคาร) จะกระทบความจุที่เก็บข้อมูล/ไฟล์ในเครื่องและประสิทธิภาพของคิวรอซิงค์ในเครื่องตาม NFR-02
...
7. **จำนวนครั้งสูงสุดที่ยอมให้กรอกรหัส PIN ผิดติดกันก่อนบังคับล้าง credential ยังไม่กำหนดตัวเลข (spec ข้อสมมติ 11, NFR-11) — เพิ่มเข้ามา 2026-09-03** — สถาปัตยกรรมนี้สมมติว่า Field Client App ต้องมีค่าจำนวนครั้งสูงสุดที่**ตั้งค่าได้ (configurable)** เช่นเดียวกับอายุ session ของ NFR-09 (ข้อสมมติ 2) ไม่ hardcode ตัวเลขคงที่ไว้ในโค้ด เพื่อให้ปรับเป็นตัวเลขจริงได้ทันทีที่ผู้ใช้ยืนยัน (คาดว่าอยู่ในช่วง 5-10 ครั้งตาม spec) โดยไม่ต้องแก้โครงสร้างสถาปัตยกรรม ถ้าสมมติผิด (เช่นต้องปรับจำนวนครั้งตามบทบาท/ระดับความเสี่ยงของพื้นที่ปฏิบัติงานที่ต่างกัน) ผลกระทบควรจำกัดอยู่ที่ค่า config และ logic นับจำนวนภายใน Field Client App เท่านั้น ไม่กระทบ component อื่น
...
- **กลไกยืนยันตัวตน/ออก session ที่เป็นรูปธรรม** (รวมถึงรูปแบบการเก็บ credential ที่แคชไว้บนอุปกรณ์อย่างปลอดภัยตาม NFR-09 และกลไกนับจำนวนครั้ง/ล้าง credential ที่แยกพื้นที่จัดเก็บออกจากข้อมูลแบบสำรวจตาม NFR-11 — ดู [[#6.8 แยกพื้นที่จัดเก็บ credential/PIN ออกจากข้อมูลแบบสำรวจในเครื่อง (NFR-11)|§6.8]]) — Field Client Framework, Local Persistent Store และ Server-side Runtime ถูกเลือกแล้ว แต่กลไก/library เฉพาะสำหรับ auth และ secure storage ระดับ implementation ยังไม่ตัดสินใจ

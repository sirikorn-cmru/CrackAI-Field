...
| ↳ คิวรับคำขอฝั่งเซิร์ฟเวอร์ (Server-side Sync Intake Queue) | จัดลำดับ/จำกัดอัตราการประมวลผลคำขอซิงค์ที่เข้ามาพร้อมกันจากหลายอุปกรณ์/หลายทีม แทนการประมวลผลพร้อมกันทั้งหมดทันที เพื่อรองรับรูปแบบโหลดที่กระจุกตัวเป็นช่วงหลังกลับจากพื้นที่ภัยพิบัติ (NFR-07) **กลไกจัดคิว/จำกัดอัตราปิดแล้ว — จำกัดโควตาแบบ token bucket ต่ออุปกรณ์ (device_id) ร่วมกับจำกัดจำนวนตัวประมวลผลที่ทำงานพร้อมกันฝั่งเซิร์ฟเวอร์ และตอบกลับพร้อมระยะเวลาที่แนะนำให้ลองใหม่เมื่อเกินขีดจำกัด (ตัวเลขจริงยังไม่ปิด รอ load test — ดู [[#6.12 กลไกจัดคิว/จำกัดอัตราคำขอซิงค์ต่ออุปกรณ์ (NFR-07)|§6.12]])** **รับส่วนย่อย (chunk) ของไฟล์ภาพที่ทยอยส่งมาและประกอบรวมเมื่อครบ พร้อมพิจารณาว่ารอบอัปโหลดใดค้างนานเกินไปแล้ว (stale) ตามเกณฑ์ที่กำหนด (ปิดแล้ว — ดู [[#6.10 ขนาด chunk และเกณฑ์เวลา stale ของรอบอัปโหลดแบบ resumable (NFR-02)|§6.10]])** | (ภายใน Sync & Conflict Resolution Service) |
...
| NFR-07 | รองรับหลายทีมซิงค์พร้อมกันจำนวนมากหลังเกิดเหตุ | Server-side Sync Intake Queue (ส่วนหนึ่งของ Sync & Conflict Resolution Service), Backend Service | ต้องมีคิวรับคำขอฝั่งเซิร์ฟเวอร์ที่จัดลำดับ/จำกัดอัตราการประมวลผลคำขอซิงค์ที่เข้ามาพร้อมกันจากหลายอุปกรณ์/หลายทีม แทนการประมวลผลพร้อมกันทั้งหมดทันที เพื่อรองรับรูปแบบโหลดที่กระจุกตัวเป็นช่วง (ไม่ใช่โหลดสม่ำเสมอ) หลังทีมกลับจากพื้นที่ภัยพิบัติ โดยไม่ทำให้ข้อมูลเสียหายหรือระบบล่ม **(ปิดเพิ่มเมื่อ 2026-09-04)** กลไกควบคุมอัตราปิดแล้วว่าจำกัดโควตาต่ออุปกรณ์ (ไม่ใช่ต่อผู้ใช้ เพราะหนึ่งทีมอาจมีหลายอุปกรณ์) ควบคู่กับจำกัดจำนวนตัวประมวลผลที่ทำงานพร้อมกันฝั่งเซิร์ฟเวอร์ — ดู [[#6.12 กลไกจัดคิว/จำกัดอัตราคำขอซิงค์ต่ออุปกรณ์ (NFR-07)|§6.12]] — การเลือก hosting เป็นคลาวด์กลางภาครัฐ (GDCC) ทำให้การขยายกำลังเครื่องแบบทันที (auto-scaling) ทำได้ช้ากว่า cloud เอกชน จึงต้องเผื่อกำลังไว้ล่วงหน้า (capacity provisioning) แทน กลไกคิว/จำกัดอัตรานี้เองเป็นส่วนสำคัญที่ทำให้ระบบรองรับโหลดกระจุกตัวได้ภายใต้กำลังที่เผื่อไว้โดยไม่ต้องพึ่ง auto-scaling |
...
- **การรองรับโหลดกระจุกตัวภายใต้ข้อจำกัดของ hosting ที่ไม่รองรับ auto-scaling แบบทันที (NFR-07) — เพิ่มเข้ามา 2026-09-04** — เป็นความรับผิดชอบร่วมของ **คิวรับคำขอฝั่งเซิร์ฟเวอร์ (Server-side Sync Intake Queue)** (จำกัดโควตาต่ออุปกรณ์แบบ token bucket ร่วมกับจำกัดจำนวนตัวประมวลผลพร้อมกัน — ดู [[#6.12 กลไกจัดคิว/จำกัดอัตราคำขอซิงค์ต่ออุปกรณ์ (NFR-07)|§6.12]]) และ **Backend Service/Primary Data Store** (ต้องออกแบบให้ทำงานได้ภายใต้กำลังเครื่องที่เผื่อไว้ล่วงหน้า — capacity provisioning — แทนการอิงว่าโครงสร้างพื้นฐานจะขยายกำลังให้อัตโนมัติเมื่อโหลดพุ่งสูงตามที่ [[technology-stack#3. การตัดสินใจรายชั้น|technology-stack §3]] ระบุไว้ว่าคลาวด์กลางภาครัฐ (GDCC) ที่เลือกใช้ทำ auto-scaling แบบทันทีได้ช้ากว่า cloud เอกชน) — สถาปัตยกรรมนี้ไม่เคยตั้งสมมติฐานว่าจะมี auto-scaling แบบทันทีมาก่อน (การออกแบบ Server-side Sync Intake Queue เป็นกลไก backpressure/คิวรอมาตั้งแต่ §6.3 อยู่แล้ว) จึงสอดคล้องกับข้อจำกัดของ GDCC โดยไม่ต้องปรับโครงสร้าง component ใหม่
...
### 6.12 กลไกจัดคิว/จำกัดอัตราคำขอซิงค์ต่ออุปกรณ์ (NFR-07)

**บริบท (ปิดแล้วเมื่อ 2026-09-04)**: NFR-07 ต้องการรองรับโหลดกระจุกตัวจากหลายทีม/หลายอุปกรณ์พร้อมกันหลังน้ำท่วม สถาปัตยกรรมเดิมระบุเพียงแนวคิด "จัดลำดับ/จำกัดอัตรา" ไว้ในระดับหลักการ (§2, §5) แต่ยังไม่เคยปิดว่าจำกัดต่อหน่วยใด ใช้กลไกแบบใด และตอบสนองอย่างไรเมื่อเกินขีดจำกัด

**ทางเลือกหน่วยที่จำกัดโควตา**: (1) จำกัดโควตาต่ออุปกรณ์ (`device_id`) (**เลือก**) (2) จำกัดโควตาต่อผู้ใช้ (`user_id`) (3) จำกัดโควตารวมทั้งระบบแบบเดียว ไม่แยกต่อหน่วยใดๆ — เลือกแบบ (1) เพราะบริบทจริงคือหนึ่งทีมสำรวจอาจใช้หลายอุปกรณ์พร้อมกัน การจำกัดต่อผู้ใช้ (2) จะลงโทษผิดหน่วยเพราะผู้ใช้คนเดียวอาจมีหลายอุปกรณ์ที่ไม่ได้ซิงค์พร้อมกันจริง ในขณะที่การจำกัดต่ออุปกรณ์สะท้อนภาระที่แท้จริงต่อทรัพยากรเซิร์ฟเวอร์ (แต่ละอุปกรณ์คือแหล่งคำขอจริงหนึ่งจุด) ตัดตัวเลือก (3) ทิ้งเพราะจะทำให้อุปกรณ์ไม่กี่เครื่องที่ส่งถี่ผิดปกติแย่งโควตาของอุปกรณ์อื่นทั้งหมด ขัดเจตนารมณ์ของ NFR-07 ที่ต้องการให้หลายทีมซิงค์พร้อมกันได้

**ทางเลือกกลไกควบคุมอัตรา**: (1) token bucket ต่ออุปกรณ์ (**เลือก**) (2) fixed window counter ต่ออุปกรณ์ (3) sliding log ต่ออุปกรณ์ — เลือกแบบ (1) เพราะรองรับ burst แบบจำกัดได้ (อุปกรณ์ที่ออฟไลน์นานเพราะไม่มีสัญญาณสามารถสะสมโควตาและส่งได้เร็วขึ้นชั่วคราวเมื่อกลับมามีสัญญาณ) ตรงกับรูปแบบการใช้งานจริงที่อุปกรณ์ออฟไลน์เป็นเวลานานแล้วกลับมาพร้อมกัน (NFR-01 ผสาน NFR-07) มากกว่า fixed window (2) ที่เสี่ยงเกิดพฤติกรรมพุ่งกระจุกที่รอยต่อของแต่ละช่วงเวลา หรือ sliding log (3) ที่ใช้ทรัพยากรเก็บบันทึกต่อคำขอมากกว่าโดยไม่ได้ประโยชน์เพิ่มที่ NFR-07 ต้องการจริง

**ทางเลือกฝั่งประมวลผล**: (1) จำกัดจำนวนตัวประมวลผล (worker) ที่ทำงานพร้อมกันในคิวรับคำขอฝั่งเซิร์ฟเวอร์ ควบคู่กับ token bucket ต่ออุปกรณ์ (**เลือก**) (2) พึ่ง token bucket ต่ออุปกรณ์อย่างเดียวโดยไม่จำกัดจำนวน worker รวม — เลือกแบบ (1) เพราะ token bucket ต่ออุปกรณ์ป้องกันได้เฉพาะอุปกรณ์เดี่ยวที่ส่งถี่เกินไป แต่ไม่ป้องกันกรณีอุปกรณ์จำนวนมากส่งพร้อมกันในอัตราที่แต่ละเครื่องปกติแต่รวมกันเกินกำลังเซิร์ฟเวอร์ที่เผื่อไว้ล่วงหน้า (capacity provisioning ตามข้อจำกัดของ GDCC — ดู [[technology-stack#3. การตัดสินใจรายชั้น|technology-stack §3]] แถว Hosting/Deployment target) การจำกัดจำนวน worker พร้อมกันจึงเป็นแนวป้องกันชั้นที่สองที่จำเป็นเมื่อ hosting ไม่รองรับ auto-scaling แบบทันที

**ทางเลือกการตอบสนองเมื่อเกินขีดจำกัด**: (1) ปฏิเสธคำขอชั่วคราวพร้อมระบุระยะเวลาที่แนะนำให้ลองใหม่ (**เลือก**) (2) ปฏิเสธเฉยๆ โดยไม่บอกเวลาที่ควรลองใหม่ (3) ปล่อยให้คำขอค้างรอในคิวไม่มีกำหนดโดยไม่ตอบกลับทันที — เลือกแบบ (1) เพราะคิวรอซิงค์ในเครื่อง (Local Sync Queue) ของ Field Client App ต้องมีข้อมูลเพียงพอที่จะตัดสินใจว่าควรลองซิงค์ใหม่เมื่อไหร่โดยไม่ต้องเดา/ลองรัวๆ ซึ่งจะยิ่งซ้ำเติมโหลดที่กระจุกตัวอยู่แล้ว ใช้ประโยชน์จากความหมายมาตรฐานของ REST over HTTP ที่ [[technology-stack]] ตัดสินใจไว้แล้วโดยตรง (สอดคล้องกับ pattern เดิมของเอกสารนี้ที่อ้างชื่อ stack เฉพาะจุดที่ทำให้ผู้อ่านเข้าใจเปลี่ยนไปอย่างมีนัยสำคัญ) ตัดตัวเลือก (2) ทิ้งเพราะจะทำให้ client เดาจังหวะลองใหม่เอง เสี่ยงซ้ำเติมโหลด ตัดตัวเลือก (3) ทิ้งเพราะขัดกับหลักการแจ้งสถานะทันทีแทนที่จะปล่อยให้รอไม่มีกำหนด

**ตัวเลขอัตราจริง (ขนาด token bucket, จำนวน worker สูงสุด, ระยะเวลาที่แนะนำให้ลองใหม่) ยังไม่ปิดในรอบนี้ — ต้องจูนจาก load test ภายหลังที่มีสภาพแวดล้อมรันจริงบน GDCC ห้ามเดาตัวเลขในรอบนี้**

**ผลกระทบต่อการออกแบบ**: คิวรับคำขอฝั่งเซิร์ฟเวอร์ (Server-side Sync Intake Queue) ต้องเก็บสถานะโควตาต่ออุปกรณ์และจำนวน worker ที่ทำงานอยู่ ณ ขณะนั้น — ไม่กระทบ component diagram หรือ data flow diagram เดิม เพราะทั้งสองไม่เคยระบุกลไกควบคุมอัตราแบบเจาะจงมาก่อน (คงความเป็น conceptual)

### 6.13 นโยบายเก็บรักษาและเตรียมกลไก archive ของ audit trail (NFR-05)

**บริบท (ปิดแล้วเมื่อ 2026-09-04)**: ข้อสมมติที่ 6 ใน §7 (เดิม) ตั้งสมมติไว้ตามข้อความ spec ว่า "เก็บไว้ตลอดอายุของระเบียนที่เกี่ยวข้อง ไม่มีการลบอัตโนมัติในเฟสนี้" โดยไม่รู้ว่าต้องเตรียมโครงสร้างรองรับ archive ในอนาคตไว้ล่วงหน้าหรือไม่ ตอนนี้ผู้ใช้ปิดประเด็นนี้แล้ว

**ทางเลือกที่พิจารณา**: (1) เก็บถาวรไม่ลบอัตโนมัติ และไม่ออกแบบกลไก archive ใดๆ ไว้ล่วงหน้า รอมีความต้องการจริงค่อยออกแบบทีหลัง (2) เก็บถาวรไม่ลบอัตโนมัติ แต่เตรียมกลไก archive ไว้ล่วงหน้าโดยยังไม่เปิดใช้งาน (**เลือก**) (3) กำหนดระยะเวลาเก็บรักษาที่แน่นอนและ archive/ลบอัตโนมัติทันทีตามระยะเวลานั้น

เลือกแบบ (2) เพราะหน่วยงาน (กรมโยธาธิการและผังเมือง) ยังไม่ได้กำหนดระยะเวลาตามระเบียบงานสารบรรณจริง — ตัดตัวเลือก (3) ทิ้งทันทีเพราะจะเป็นการเดาตัวเลขที่ยังไม่มีใครยืนยัน เสี่ยงลบ/archive ข้อมูลที่หน่วยงานต้องการเก็บไว้จริงก่อนกำหนด แต่ก็ไม่เลือก (1) เฉยๆ เพราะ [[technology-stack#6. เกณฑ์ที่จะทำให้ต้องทบทวนการตัดสินใจนี้ใหม่|technology-stack §6 ข้อ 6]] ระบุไว้แล้วว่าปริมาณ audit log ที่สะสมไม่มีกำหนดจะกระทบประสิทธิภาพการค้นหาของ FR-33 ในอนาคต — การเตรียมกลไก archive ไว้ล่วงหน้า (แม้ยังไม่เปิดใช้) ทำให้เมื่อหน่วยงานกำหนดระยะเวลาจริงในอนาคต ระบบไม่ต้องออกแบบโครงสร้างข้อมูลใหม่ทั้งหมด เพียงเปิดใช้กลไกที่เตรียมไว้

**ผลกระทบต่อการออกแบบ**: **Primary Data Store** ต้องมีแอตทริบิวต์สถานะ archive สำรองไว้ในโมเดลข้อมูลของ audit log (เพิ่มเข้ามา 2026-09-04 — ดู §2 แถว Primary Data Store) แต่**ยังไม่เปิดใช้งานจริง**ในเฟสนี้ (ไม่มี logic ใดอ่าน/เขียนสถานะนี้จนกว่าจะมีนโยบายที่แน่นอน) — **ระยะเวลาเก็บรักษาก่อนเปิดใช้กลไก archive ยังไม่ปิดเป็นตัวเลข รอหน่วยงานกำหนดตามระเบียบงานสารบรรณ** (ยังคงเป็นประเด็นเปิดใน §8) ไม่กระทบ component diagram/data flow diagram เพราะเป็นรายละเอียดระดับแอตทริบิวต์ ไม่ใช่ flow หรือ component ใหม่
...
6. **นโยบายเก็บรักษาประวัติการแก้ไข (audit trail) — ปิดบางส่วนแล้วเมื่อ 2026-09-04 (spec ข้อสมมติ 10, FR-32/FR-33, NFR-05)**: ผู้ใช้ยืนยันนโยบายแล้วว่า **เก็บถาวรไม่ลบอัตโนมัติ แต่ต้องออกแบบกลไก archive เตรียมไว้ล่วงหน้า** (เพิ่มแอตทริบิวต์สถานะ archive ในโมเดลข้อมูล แต่ยังไม่เปิดใช้งานจริง — ดู [[#6.13 นโยบายเก็บรักษาและเตรียมกลไก archive ของ audit trail (NFR-05)|§6.13]]) เดิมสถาปัตยกรรมนี้เคยสมมติตามข้อความ spec ว่า "เก็บไว้ตลอดอายุของระเบียนที่เกี่ยวข้อง ไม่มีการลบอัตโนมัติในเฟสนี้" โดยไม่รู้ว่าต้องเตรียมโครงสร้างรองรับ archive ล่วงหน้าหรือไม่ ซึ่งตอนนี้ปิดแล้วว่า**ต้องเตรียมไว้** — ผลกระทบเดิมที่เคยระบุไว้ทั้งหมด (กระทบ Primary Data Store ที่ต้องเพิ่มกลไก archive/purge และกระทบประสิทธิภาพการค้นหาของ FR-33) **ไม่จำเป็นอีกต่อไปในส่วนที่ว่าไม่มีการเตรียมโครงสร้างใดๆ ไว้เลย** เพราะตอนนี้มีแอตทริบิวต์ archive สำรองไว้แล้ว แต่ยังมีผลกระทบเหลืออยู่เฉพาะ**ระยะเวลาเก็บรักษาก่อนเปิดใช้กลไก archive ที่เป็นตัวเลขจริง** ซึ่งยังไม่ปิด (รอหน่วยงานกำหนดตามระเบียบงานสารบรรณ — ดู §8) ถ้าระยะเวลาที่หน่วยงานกำหนดในอนาคตสั้นกว่าที่คาดไว้มาก อาจต้องเร่งเปิดใช้กลไก archive ที่เตรียมไว้เร็วกว่าแผน
...
**ปรับปรุงเพิ่มเติม 2026-09-04**: ปิดเพิ่มอีก 1 ชั้นเป็น 6 ชั้นรวม — **Protocol/Web Framework → REST over HTTP + Fastify** (ปิดแล้ว ดู [[technology-stack#3. การตัดสินใจรายชั้น|technology-stack §3]]) ทั้ง framework ระดับ workflow, library ของ Local Persistent Store และ Protocol/Web Framework ถูกตัดสินใจครบแล้วในรอบนี้ นอกจากนี้ผู้ใช้ยังยืนยันการตัดสินใจเชิงสถาปัตยกรรมเพิ่มอีก 3 ข้อในรอบเดียวกัน ได้แก่ กลยุทธ์การจัดการความขัดแย้งของข้อมูล (NFR-03 — ดู [[#6.9 กลยุทธ์การจัดการความขัดแย้งของข้อมูล เมื่อ merge อัตโนมัติไม่ได้ (NFR-03)|§6.9]]), ขนาด chunk และเกณฑ์เวลา stale ของรอบอัปโหลดแบบ resumable (NFR-02 — ดู [[#6.10 ขนาด chunk และเกณฑ์เวลา stale ของรอบอัปโหลดแบบ resumable (NFR-02)|§6.10]]) และรูปแบบ token/session ยืนยันตัวตนฝั่งเซิร์ฟเวอร์ (NFR-08, NFR-09 — ดู [[#6.11 รูปแบบ token/session ยืนยันตัวตน — opaque token แบบมีสถานะ ไม่ใช่ JWT แบบไร้สถานะ (NFR-08, NFR-09)|§6.11]]) รายการที่เคยเปิดไว้เรื่อง "รูปแบบ/โปรโตคอลการซิงค์ข้อมูลระหว่าง Field Client กับ Sync Service (รวมถึงกลยุทธ์ merge conflict)" จึงปิดไปแล้วทั้งหมดในระดับที่สถาปัตยกรรมนี้ต้องพูดถึง (รายละเอียด wire-level/schema ของแต่ละ chunk เป็นระดับ operation contract ของ [[api-spec]] ไม่ใช่ระดับสถาปัตยกรรมนี้อีกต่อไป) และรายการ "กลไกยืนยันตัวตน/ออก session ที่เป็นรูปธรรม" ปิดไปแล้วบางส่วน (ดูรายละเอียดในรายการย่อยด้านล่าง) **ปรับปรุงเพิ่มเติม 2026-09-04 (รอบสอง)**: ปิดเพิ่มอีก 1 ชั้นเป็น **7 ชั้นรวม** — **Hosting/Deployment target → คลาวด์กลางภาครัฐ (GDCC)** (ดู [[technology-stack#3. การตัดสินใจรายชั้น|technology-stack §3]] และ [[technology-stack#4.2 Hosting/Deployment target|technology-stack §4.2]]) พร้อมข้อจำกัดสำคัญที่กระทบ **NFR-07**: การขยายกำลังเครื่องแบบทันที (auto-scaling) ทำได้ช้ากว่า cloud เอกชน จึงต้องเผื่อกำลังไว้ล่วงหน้า (capacity provisioning) แทน — ผลกระทบต่อการออกแบบดูที่ [[#5.5 Cross-cutting concerns|§5.5]] และ [[#6.12 กลไกจัดคิว/จำกัดอัตราคำขอซิงค์ต่ออุปกรณ์ (NFR-07)|§6.12]] รายการ **Hosting/Deployment target** จึงถูกย้ายออกจากรายการ "ยังไม่มีคำตอบ" ด้านล่างแล้ว (เดิมเคยอยู่ในรายการนั้น) นอกจากนี้ผู้ใช้ยังปิดการตัดสินใจเชิงสถาปัตยกรรมเพิ่มอีก 2 ข้อในรอบเดียวกัน: กลไกจัดคิว/จำกัดอัตราคำขอซิงค์ต่ออุปกรณ์ของ Server-side Sync Intake Queue (NFR-07 — ดู [[#6.12 กลไกจัดคิว/จำกัดอัตราคำขอซิงค์ต่ออุปกรณ์ (NFR-07)|§6.12]]) และนโยบายเก็บรักษา/เตรียมกลไก archive ของ audit trail (NFR-05 — ดู [[#6.13 นโยบายเก็บรักษาและเตรียมกลไก archive ของ audit trail (NFR-05)|§6.13]]) — ตัวเลขที่ยังไม่ปิด (อัตรา rate limit ที่แน่นอน, ระยะเวลาเก็บรักษาก่อนเปิดใช้ archive) ยังอยู่ในรายการด้านล่าง

รายการที่เหลือด้านล่างคือสิ่งที่**ยังไม่มีคำตอบจริง**แม้ tech stack หลักและการตัดสินใจเชิงสถาปัตยกรรมรอบนี้จะถูกปิดไปแล้วบางส่วน:

- **Runtime ของโมเดล AI วิเคราะห์รอยร้าว + ขนาด/ความละเอียดภาพ — เลื่อนโดยตั้งใจ (deferred by decision, 2026-09-04)** ไม่ใช่ "ยังไม่มีคำตอบ" แบบค้างคาเพราะขาดข้อมูล (บล็อกเฉพาะ Phase 3 เพราะยังไม่มีโมเดล/ชุดข้อมูลจริง ไม่บล็อก Phase 1-2 — แนวทางชั่วคราวของ Phase 2 คือเก็บไฟล์ภาพต้นฉบับความละเอียดเต็มไว้เสมอ เพราะ downscale ภายหลังทำได้แต่ upscale ไม่ได้) — ดู [[technology-stack#4.1 Runtime ของโมเดล AI วิเคราะห์รอยร้าว + ขนาด/ความละเอียดภาพ (เลื่อนโดยตั้งใจ — deferred by decision, 2026-09-04)|technology-stack §4.1]] และดู [[#6.1 ตำแหน่งการทำงานของ AI วิเคราะห์รอยร้าว|§6.1]] ของเอกสารนี้ประกอบ เพราะ Field Client Framework ที่เลือกไว้ (bare React Native) มีข้อจำกัดด้าน on-device inference ที่ต้องพิจารณาซ้ำเมื่อถึงเวลาตัดสิน
...
- **รูปแบบการจัดทำดัชนี/กลยุทธ์การค้นหาที่มีประสิทธิภาพสำหรับการค้นหา/กรองประวัติทั่วทั้งระบบ (FR-33)** เมื่อปริมาณ audit log สะสมมากขึ้นเรื่อยๆ โดยไม่มีการลบอัตโนมัติ (ดูข้อสมมติที่ 6 ใน §7) — เพิ่มเข้ามา 2026-09-02 — **ปิดบางส่วนแล้วเมื่อ 2026-09-04**: เพิ่ม index `(performed_by_user_id, performed_at)` รองรับตัวกรอง "ผู้แก้ไข" ของ FR-33 แล้วที่ [[db-spec]] (ครบทั้งสามเงื่อนไขร่วมกับ index เดิมของอาคาร/ทีมและช่วงเวลา+ประเภทการกระทำ) — **ยังไม่ปิด**: กลยุทธ์รองรับปริมาณข้อมูลจำนวนมากในระยะยาวเมื่อไม่มีการลบอัตโนมัติ (เช่น read replica/partition ตามที่เคยเปิดไว้) โดยเฉพาะเมื่อรวมกับนโยบาย archive ที่เตรียมไว้แต่ยังไม่เปิดใช้ (ดู §6.13) — ดู [[technology-stack#6. เกณฑ์ที่จะทำให้ต้องทบทวนการตัดสินใจนี้ใหม่|technology-stack §6 ข้อ 6]] ประกอบ
- **ระยะเวลาเก็บรักษาก่อนเปิดใช้กลไก archive ของ audit trail (NFR-05) — เพิ่มเข้ามา 2026-09-04** — นโยบายหลัก (เก็บถาวรไม่ลบอัตโนมัติ + เตรียมกลไก archive ไว้ล่วงหน้า) ปิดแล้ว (ดู [[#6.13 นโยบายเก็บรักษาและเตรียมกลไก archive ของ audit trail (NFR-05)|§6.13]]) แต่ตัวเลขระยะเวลาที่แน่นอนก่อนเปิดใช้ archive ยังไม่ปิด รอหน่วยงาน (กรมโยธาธิการและผังเมือง) กำหนดตามระเบียบงานสารบรรณ — **ห้ามเดาตัวเลขจนกว่าจะมีคำตอบจากหน่วยงาน**

...
ระบบนี้มีข้อจำกัดเชิงสถาปัตยกรรม 4 ข้อที่กำหนดทิศทางการออกแบบทั้งหมด (สืบเนื่องจาก [[feature-list|ฟีเจอร์ 13 และ 14]]):
...
| **Management Web Client** | ตรวจทาน/ลงลายมือชื่อดิจิทัล/มอบหมายงาน, จัดการผู้ใช้และสิทธิ์, ดู dashboard, ค้นหา/กรอง, ส่งออกรายงาน — ต้องมีสัญญาณอินเทอร์เน็ตในการใช้งาน | หัวหน้าผู้สำรวจ, ผู้ดูแลระบบ, หน่วยงานส่วนกลาง/ผู้บริหาร |
| **Backend Service** | ยืนยันตัวตน/ออก session, บังคับสิทธิ์ตามบทบาท (RBAC), business logic ของทุกฟีเจอร์ (มอบหมายงาน, รับรองผล, จัดการผู้ใช้, dashboard, ส่งออกรายงาน, ค้นหา/กรอง), บันทึก audit trail กลาง | ทุกบทบาท (ผ่าน client ทั้งสองฝั่ง) |
| **Sync & Conflict Resolution Service** | รับข้อมูล/ไฟล์ที่ค้างจาก Local Sync Queue ของหลายอุปกรณ์พร้อมกัน ตรวจจับและแก้ไขความขัดแย้งของข้อมูลชุดเดียวกันที่มาจากหลายอุปกรณ์ รองรับปริมาณการซิงค์พร้อมกันสูงหลังเกิดเหตุ | ระบบภายใน (ไม่มีผู้ใช้เรียกตรง) |
...
| **Primary Data Store** | เก็บข้อมูลเชิงโครงสร้างหลัก: อาคาร, ผลประเมินความเสียหาย, ผู้ใช้/บทบาท, งานมอบหมาย, ผลรับรอง/ลายเซ็น, audit log | เข้าถึงผ่าน Backend Service เท่านั้น |
...
flowchart LR
    ...
    FCQueue -->|ซิงค์เมื่อมีสัญญาณ| SYNC
    SYNC --> DB
    SYNC --> MEDIA
    SYNC --> BE
    FC -.->|เข้าสู่ระบบ/ยืนยันตัวตนใหม่เมื่อมีสัญญาณ| BE
    MC -->|เรียกใช้งานทุกฟังก์ชัน| BE
    BE --> DB
    BE --> MEDIA
...
อ้างอิง journey เต็มที่ [[user-journey]] — เลือกทำ diagram สำหรับ 3 รูปแบบ flow ที่มีผลต่อสถาปัตยกรรมชัดเจนและแตกต่างกัน (journey ผู้ดูแลระบบและหน่วยงานส่วนกลางมี flow เชิงสถาปัตยกรรมแบบเดียวกัน คือ "เรียก Management Web Client → Backend Service → Primary Data Store แบบออนไลน์" จึงรวมเป็น diagram เดียว)
...
sequenceDiagram
    ...
    Queue->>Sync: ส่งข้อมูล/ไฟล์ภาพที่ค้างอยู่ (NFR-02)
    Sync->>Sync: ตรวจจับและจัดการความขัดแย้งของข้อมูล (NFR-03, NFR-07)
...
| NFR-07 | รองรับหลายทีมซิงค์พร้อมกันจำนวนมากหลังเกิดเหตุ | Sync & Conflict Resolution Service, Backend Service | ต้องออกแบบให้รองรับปริมาณคำขอซิงค์พร้อมกันสูงโดยไม่ทำให้ข้อมูลเสียหายหรือระบบล่ม (จัดคิว/จำกัดอัตราเชิงหลักการ) |
...
NFR ครบทั้ง 9 ตัวถูก map ไปยัง component แล้ว ไม่มีตัวใดขาดหาย
...
- [[feature-list]] — รายการฟีเจอร์ทั้ง 14 รายการที่สถาปัตยกรรมนี้ต้องรองรับ
